...
class View extends Component {
    constructor(props) {
        ...
        this.initialState = {
            file: {}, redirectUpdate: false,
            paymentType: "n", updateStatusCode: 1,
            tempUpdateStatusCode: 1,
            PMRINDEX: null,
            currentDiseaseName: "",
            drag: false, toggleDropDownForStatus: false,
            ...
            diseaseId: 0, Tissue: 0, currentTissue: 0, diseaseCategoryId: 0,
            ...
            steps: [{
                ...
                intro: "This page contains all the patients data and tracks them through our process.",
            ...
            },
            {
                ...
                intro: "You can edit or update Disease Description details by clicking on this icon."
            },

            {
                ...
                intro: "You can update assigned practitioner for this patient by clicking on this icon."
            },
            {
                element: "#CreatePatientNote",
                tooltipClass: "cssClassName1",
                title: "Creat Note",
                intro: "This is used to capture detailed information for this patient, like; lab sample status, delays in process, unique actions, etc."
            },
            ...
            },
            {
                ...
                intro: "You can generate patient's report by clicking on this button."
            },
            {
                element: "#PMRandFinalReportGPreventionR",
                tooltipClass: "cssClassName1",
                title: "Generate Prevention Report",
                intro: "You can generate prevention report by clicking on this button."
            },
            {
                element: "#PMRandFinalReportGLongevityR",
                tooltipClass: "cssClassName1",
                title: "Generate Longevity Report",
                intro: "You can generate longevity report by clicking on this button."
            },
            {
    ...
    onAfterChange = newStepIndex => {
        ...
        if (newStepIndex === 15) {
            const element = document.querySelector("#PMRandFinalReportPatientFileName")
        ...
        if (newStepIndex === 16) {
            ...
            const element = document.querySelector("#PMRandFinalReportPMRFileApproved")
    ...
    handleUpdateStatusChange = (value, code) => {
        ...
        if (code == this.state.updateStatusCode) { return; } else {
    ...
    handleSubmitStatus = () => {
        this.setState({ loading: true, }, () => {
            ...
            let url = apiroute + "/api/BE_PatientAccessionMapping/ChangeAccessionStatus";
            const data = {
            ...
            axiosInstance.post(url, data, {
                ...
                // console.log({ data })
                if (data.flag) {
                    this.setState({ loading: false, currentStatus: this.state.newStatus, showUpdateStatusModel: false, updateStatusCode: this.state.tempUpdateStatusCode }, () => {
                        const param = this.props.match.params;
                        this.getData(param.id, param.aid);
                    })
                    toast.success(data.message)
                } else {
                    toast.error(data.message || "something went wrong while updating status , please try again")
                    this.setState({ loading: false, })
                }
            }).catch((e) => {
                this.setState({ loading: false, })
                toast.error(e?.message || "something went wrong while updating status , please try again")
            })
        })
    }
    ...
    validateNotesData = (errors) => {
        ...
        // }
        if (this.state.inputTextforNotes == undefined || this.state.inputTextforNotes == "") {
    ...
    togglePrescriptionFile = (e, id) => {
        ...
        this.setState({
            DeletemodalforPrescriptionFile: !this.state.DeletemodalforPrescriptionFile,
    ...
    deletePatientNote(e, patientNoteId) {
        ...
        const apiroute = window.$APIPath;
        const url = apiroute + '/api/BE_PatientNotes/Delete?id=' + patientNoteId + '&userId=' + userToken.userId;
    ...
    EditNotes(e, patAccessId, notesId, notesTitle, sectionId, notes, assignedToUserId, createdBy) {
        this.setState({ loading: true });
        ...
        // this.OrganizationUserDataForNotes();
        let url = apiroute + "/api/BE_PatientNotes/GetById?patAccessId=" + patAccessId + "&notesId=" + notesId;
    ...
    DeleteComment(e, commentId, patientNotesId) {
        ...
        let url = apiroute + "/api/BE_Comments/Delete?id=" + commentId + "&userId=" + patientNotesId
        // let data = JSON.stringify({
        //   "commentId": commentId,
        //   "patientNotesId": parseInt(patientNotesId),
    ...
    submitDisgnOrderStatus = (isUpdate) => {
        return new Promise(async (resolve) => {
            try {
                ...
                let data = {}
                let url = ''
                let x = new Date(this.state.paymentType === 'y' ? this.state.analysisOrderStatus.paymentRecived.date : this.state.designOrderStatus.orderSubmited.date)
                ...
                if (isUpdate) {
                    ...
                    data = {
                        "orderHistoryId": Number(this.state.basicInfo?.designOrderStatus?.orderHistoryId),
                        orderDate: x,
                        "updatedBy": uid,
    ...
    isSubmitDisabledForSampleLab = (labId, labType) => {
        try {
            ...
            // debugger;
            let sampleIds = Object.keys(allLabs[labId]).filter(ele => !submittedSamples.includes(Number(ele)))

            // debugger;
            let isAllSamplesFalsy = sampleIds.every((sampleId) => {
                let sample = allLabs[labId][sampleId]
                // debugger;
                return sample.checked == false && sample.date == '' && sample.isNa == false
            ...
            if (isAllSamplesFalsy) return isAllSamplesFalsy;
                ...
                return sampleIds.filter(sampleId => {
                    let sample = allLabs[labId][sampleId]
                    return sample.checked && sample.date === '' || !sample.checked && sample.date !== ''
                }).length > 0
            }
        } catch (error) {
            return false
    ...
    isAllSamplesSubmittedForLab = (labId) => {
        try {
            ...
            let filteredSamples = allSamples.filter((ele) => ele.ngsLaboratoryId == labId)
            let submitedSamples = filteredSamples.filter((ele) => ele.sampleTypeReceived)
    ...
    getProcessStatus = () => {
        ...
        if (Pfailed) {
            ...
            return <p className="px-lg-5 text-bold" style={{ color: "orange", fontWeight: "bold" }}>
                ...
                <a
                    className="ml-1"
                    target="_blank" href={Preason?.slice?.(Preason?.search("http"))}
                >
                    {Preason}
                </a>
            </p>
    ...
    isAllSamplesUploadedToBucket = (labId) => {
        try {
            ...
            let filteredSamples = allSamples.filter((ele) => ele.ngsLaboratoryId == labId)
            let submitedSamples = filteredSamples.filter((ele) => ele.sampleTypeResultStatus)
            return submitedSamples.length === filteredSamples.length
        } catch (error) {
            return false
        }
    ...
    isSampleUploadedToBucket = (labId, collectionId) => {
        try {
            let samples = this.state.basicInfo?.labReceivedResultStatus.findIndex((ele) => {
                return ele.ngsLaboratoryId == labId && ele.ngsLaboratoryPatientActivityId == collectionId && ele.sampleTypeResultStatus
    ...
    isAnalysisOrdeDisabled = () => {
        try {
            ...
            let orderSubmited = this.state.basicInfo.analysisOrderStatus?.orderSubmited
    ...
    isDesignOrderPoDisabled = () => {
        ...
        let invoiceSentToClinic = this.state.basicInfo?.designOrderStatus?.invoiceSend
        if (!checked) { return true }
    ...
    isPeptideDisabledForPMR = () => {
        ...
        let clinicsentToPhr = this.state.basicInfo?.peptideStatus?.clinicSendPMRToPharmacy
        if (!clinicsentToPhr) {
    ...
    isPeptideDisabledForClinic = () => {
        ...
        let peptidesReceivedByClinic = this.state.basicInfo?.peptideStatus?.peptidesReceivedByClinic
        if (!checked) { return true }
    ...
    isPeptideDisabledForrecievedByClinic = () => {
        ...
        let clinicSendPMRToPharmacy = this.state.basicInfo?.peptideStatus?.clinicSendPMRToPharmacy
        let treatmentStarted = this.state.basicInfo?.peptideStatus?.treatmentStarted
        if (!cheked) { return true }
        if (!treatmentStarted) {
    ...
    applySameDateToLabReceived = (labId, value, labtype, filtered = []) => {
        ...
        keys.filter((ele) => clone[ele].checked && filtered.includes(ele)).map((ele) => {
    ...
    displayApplyDate = (labId, boxid, checkIfDisabled) => {
        ...
        if ((key == boxid && filter.length !== keys.length && cheked.length >= 2) || key == boxid && !isEqual) {
            return (<Col style={{ display: "contents" }}>
                <div onClick={() => this.applySameDateToLabReceived(labId, lab[boxid].date, 'selectedLabs', keys)} style={{ cursor: "pointer", textDecoration: "underline" }}>
    ...
    displayApplyDateForBucket = (labId, boxid, checkIfDisabled) => {
        ...
        if ((key == boxid && filter.length !== keys.length && cheked.length >= 2) || key == boxid && !isEqual) {
            return (<Col>
                <div onClick={() => this.applySameDateToLabReceived(labId, lab[boxid].date, 'selectedLabsForBucket', keys)} style={{ cursor: "pointer", textDecoration: "underline" }}>
                    Apply same date to selected
                </div>
            </Col>);
        } else return null
        ...
        return new Promise(async (resolve) => {
            ...
            try {
                ...
                if (!notesData || !revisedData || !version) {
                    ...
                    if (!version) { err.version = "this field is required" }
                    this.setState({
                        errors: err
                    })
                    return
    ...
    submitPatientSpecimenFromSelectedLab = (labid) => {
        return new Promise(async (resolve) => {
            ...
            id:num
                ...
                resolve(true);
                // console.log(error);
                toast.error(error?.data?.message || "Error while updating Patient Specimen Received at Lab");
            }
        });
    };
        ...
        return new Promise(async (resolve) => {
            ...
            id:num
                ...
                let data = labKeys.map((ele) => {
                    ...
                    let sampleNotApplicableStatus = this.state.basicInfo?.labReceivedResultStatus.filter((Cele) => {
                ...
                flag ? toast.success(result?.data?.message) : toast.error(result.data.message)
                resolve(true);
                // console.log(data);
            } catch (error) {
                resolve(true);
    ...
    diasbledCheckboxForSpecimenCollectionSampleLabs = (labId, collectionId) => {
        try {
            let samples = this.state.basicInfo?.specimenReceivedStatus.findIndex((ele) => {
                return ele.ngsLaboratoryId == labId && ele.ngsLaboratoryPatientActivityId == collectionId && ele.sampleTypeReceived
    ...
    disablesNaData = (labid, sampleid) => {
        ...
        if (
            ...
            let index = labdata.findIndex((e) => { return e.ngsLaboratoryId == labid && e.ngsLaboratoryPatientActivityId == sampleid })
            if (index !== -1) {
        ...
        axiosInstance
            ...
            .then((result) => {
                ...
                if (result.data.flag) {
                    ...
                    const updateStatusCode = pData.patientAccessionMapping?.accessionStatusId
                    ...
                    this.setState({
                        ...
                        currentStatus, updateStatusCode, tempUpdateStatusCode: updateStatusCode,
                        paymentType,
                        patientHIReports,
                        PatientAdmeToxReports,
                        patientPMRReports,
                        ...
                        currentDiseaseCategoryId: pData.patientDisease?.[0]?.diseaseCategoryId,
                        ...
                        designOrderStatus: {
                            orderSubmited: {
                                ...
                                date: pData?.designOrderStatus?.orderSubmitedDate ? pData?.designOrderStatus?.orderSubmitedDate == '0001-01-01T00:00:00' ? "" :
                        ...
                        peptides: {
                            pmrSentToClinic: {
                                checked: pData?.peptideStatus?.pmrSendToCilnic, date: pData?.peptideStatus?.pmrSendToCilnicDate ? new Date(pData?.peptideStatus?.pmrSendToCilnicDate) : '',
                            },
                            clinicSentPmrToPharmacy: {
                                checked: pData?.peptideStatus?.clinicSendPMRToPharmacy, date: pData?.peptideStatus?.clinicSendPMRToPharmacyDate ? new Date(pData?.peptideStatus?.clinicSendPMRToPharmacyDate) : ""
                            },
                            peptidesReceivedByClinic: {
                                checked: pData?.peptideStatus?.peptidesReceivedByClinic, date: pData?.peptideStatus?.peptidesReceivedByClinicDate ? new Date(pData.peptideStatus?.peptidesReceivedByClinicDate) : ""
                            },
                            treatMentStarted: {
                                checked: pData?.peptideStatus?.treatmentStarted, date: pData?.peptideStatus?.treatmentStartedDate ? new Date(pData?.peptideStatus?.treatmentStartedDate) : ""
                            },
                        },
                    ...
                    if (this.state.basicInfo?.designActivity?.currentStatus == "Design Started") {
                        this.setState({
                            spinStatus: true
                        })
                    } else {
    ...
    isAllLabSamplesAssignedForVerify = () => {
        if (this.state.basicInfo?.designActivity == null) { return true; }

        const verifyStatus = String(this.state.basicInfo?.designActivity?.verifyStatus).toLowerCase()
        ...
        let labresultdata = this.state?.LabsStatusMap.every((value) => value === true)
    ...
    handleClinicSentPmrToPharmacy = () => {
        return new Promise(async (resolve) => {
            try {
                const apiroute = window.$APIPath;
                const url = apiroute + '/api/BE_DesignOrder/UpdateClinicSendPMRToPharmacy';
                let uid = 0;
                var userToken = JSON.parse(localStorage.getItem("AUserToken"));
                if (userToken != null) {
                    uid = userToken.userId == null ? 0 : userToken.userId;
    ...
    handlePeptidesReceviedByclinic = () => {
        return new Promise(async (resolve) => {
            try {
                const apiroute = window.$APIPath;
                const url = apiroute + '/api/BE_DesignOrder/UpdatePeptidesReceivedByClinic';
                let uid = 0;
                var userToken = JSON.parse(localStorage.getItem("AUserToken"));
                if (userToken != null) {
                    uid = userToken.userId == null ? 0 : userToken.userId;
    ...
    isPeptidesDisabled = (name) => {
        try {
            ...
            if (name == 'pmrSendToCilnic' && this.state.basicInfo.peptideStatus == null) {
                // alert(name)

                return false
            } else
                return this.state.basicInfo.peptideStatus[name]
        } catch (error) {
            return false
        }
    }
    ...
    DeleteReportfile() {
        ...
        const url = apiroute + '/api/BE_PatientReport/DeletePatientReportFile?id=' + id;
        // alert(String(id))
        // let data = {
        //   id: id,
        // }
    ...
    handleShowDiseaseDescription = () => {
        let DiseaseDescData = this?.state?.basicInfo?.patientAccessionMapping?.diseaseDesc
    ...
    VerifyData = () => {
        return new Promise(async (resolve) => {
            try {
                ...
                let url =
                    apiroute + "/api/BE_DesignActivity/UpdateDesignActivityVerificationStarted?id=" + id;
    ...
    reCallPipeLine = (designActivityId) => {
        return new Promise(async (resolve) => {
            try {
                ...
                let url = apiroute + "/api/BE_DesignActivity/RecallPipeline?id=" + designActivityId;
                let res = await axiosInstance.get(url);
                if (res.data.flag) {
                    toast.success(res.data.message)
                } else {
            ...
            orderSubmitted: (isUpdate) => {
                return new Promise(async (resolve) => {
                    try {
                        let url = ''
                        let data = {}
                        let x = new Date(this.state.analysisOrderStatus.orderSubmitted.date)
                        let hoursDiff = x.getHours() - x.getTimezoneOffset() / 60;
                        let minutesDiff = (x.getHours() - x.getTimezoneOffset()) % 60;
                        x.setHours(hoursDiff);
                        x.setMinutes(minutesDiff);
                        if (isUpdate) {
                            ...
                            data = {
                                "orderHistoryId": Number(this.state.basicInfo?.analysisOrderStatus?.orderHistoryId),
                                orderDate: x,
                                "updatedBy": uid
                            }
                        ...
                        if (flag) toast.success(response.data.message); else toast.error(response.data.message)
                        resolve(true);
                        // .then(() => {
                        //   //
                        // })
    ...
    handleAnalysisOrderChange = (name) => {
        if (name == "paymentRecived" && this.state?.analysisOrderStatus?.paymentRecived?.checked === false) {
            this.setState({
                showPaidStatus: true
            })
    ...
    invoiceSentToClinic = (id) => {
        return new Promise(async (resolve) => {
            try {
                ...
                let x = new Date(this.state.paymentType === 'y' ? this.state.analysisOrderStatus.paymentRecived.date : this.state.designOrderStatus.invoice.date)
                let hoursDiff = x.getHours() - x.getTimezoneOffset() / 60;
                let minutesDiff = (x.getHours() - x.getTimezoneOffset()) % 60;
                x.setHours(hoursDiff);
                x.setMinutes(minutesDiff);
    ...
    paymentReceived = (id) => {
        return new Promise(async (resolve) => {
            try {
                ...
                let x = new Date(this.state.paymentType === 'y' ? this.state.analysisOrderStatus.paymentRecived.date : this.state.designOrderStatus.paymentRecived.date)
                let hoursDiff = x.getHours() - x.getTimezoneOffset() / 60;
                let minutesDiff = (x.getHours() - x.getTimezoneOffset()) % 60;
                x.setHours(hoursDiff);
                x.setMinutes(minutesDiff);
    ...
    poReceived = (id) => {
        return new Promise(async (resolve) => {
            try {
                ...
                let x = new Date(this.state.paymentType === 'y' ? this.state.analysisOrderStatus.paymentRecived.date : this.state.designOrderStatus.poReceived.date)
                let hoursDiff = x.getHours() - x.getTimezoneOffset() / 60;
                let minutesDiff = (x.getHours() - x.getTimezoneOffset()) % 60;
                x.setHours(hoursDiff);
                x.setMinutes(minutesDiff);
    ...
    handleSubmitNewDisease = (e) => {
        // debugger;
        if (this.state.newDisease && this.state.Tissue) {
    ...
    previewPdfToggle_api_bottom(trid, trfid, PType) {
        ...
        if (PType == "d1") {
            url = apiroute + "/api/BE_PatientDiagnosticFile/ViewPatientDiagnosticFile";
            data = JSON.stringify({
                trid: trid,
                trfid: trfid
            });
        ...
        if (PType == "m1") {
            url = apiroute + "/api/BE_PatientPrescriptionFile/ViewPatientPrescriptionFile";
            data = JSON.stringify({
                trid: trid,
                trfid: trfid
            });
        ...
        return (
            <div className="animated fadeIn viewPatientDetails">
                ...
                <Steps
                    ...
                    options={{
                        ...
                        , showStepNumbers: true, showBullets: false
                    }}
                ...
                />
                <button className="help text-light" id="help" type="btn" onClick={() => { this.toggleSteps() }}>
                ...
                <Row className="mb-3">
                    ...
                    <Col sm="3" lg="3" xl="3" >
                        ...
                        <div className="form-group form-inline" id="Status" style={{ width: "fit-content" }}>
                            <label htmlFor="PatientStatus" className="form-control-label mr-2"
                            >
                                Status:
                            </label>
                            {/* <Input
                ...
                >
                  ...
                  <option>Deceased</option>
                            ...
                            <Dropdown
                                ...
                                toggle={() => {
                                    this.setState({ toggleDropDownForStatus: !this.state.toggleDropDownForStatus })
                                }} isOpen={this.state.toggleDropDownForStatus}>
                                <DropdownToggle className="custom-select" style={{ "display": "flex", "alignItems": "center", "justifyContent": "center" }} >
                                    <div className="dot" style={{ "backgroundColor": this.getStatusColor() }}></div> {this.state.currentStatus}
                                </DropdownToggle>
                                <DropdownMenu container="body">
                                    <DropdownItem onClick={() => this.handleUpdateStatusChange('Open', 1)} active={this.state.updateStatusCode == 1} className="custom-status-dropdown">
                                        <div style={{ "display": "flex", "alignItems": "center", "justifyContent": "start" }}>

                                            <div className="dot" style={{ "backgroundColor": "lightgreen" }}></div>
                                            <div>
                                                Open
                                            </div>
                                        </div>
                                    </DropdownItem>
                                    <DropdownItem onClick={() => this.handleUpdateStatusChange('On Hold', 2)} active={this.state.updateStatusCode == 2} className="custom-status-dropdown">
                                        <div style={{ "display": "flex", "alignItems": "center", "justifyContent": "start" }}>
                                    ...
                                    <DropdownItem onClick={() => this.handleUpdateStatusChange('Complete', 3)} active={this.state.updateStatusCode == 3} className="custom-status-dropdown">
                                        <div style={{ "display": "flex", "alignItems": "center", "justifyContent": "start" }}>
                                    ...
                                    <DropdownItem onClick={() => this.handleUpdateStatusChange('Cancelled', 4)} active={this.state.updateStatusCode == 4} className="custom-status-dropdown">
                                        <div style={{ "display": "flex", "alignItems": "center", "justifyContent": "start" }}>
                                            <div className="dot" style={{ "backgroundColor": "gold" }}></div>
                                            <div>
                                                Cancelled
                                            </div>
                                        </div>
                                    </DropdownItem>
                                    <DropdownItem onClick={() => this.handleUpdateStatusChange('Deceased', 5)} active={this.state.updateStatusCode == 5} className="custom-status-dropdown">
                                        <div style={{ "display": "flex", "alignItems": "center", "justifyContent": "start" }}>
                                            <div className="dot" style={{ "backgroundColor": "crimson" }}></div>
                                            <div>
                                                Deceased
                                            </div>
                                        </div>
                                            ...
                                            <FormGroup>
                                                ...
                                                <div className="float-right h5">
                                                    <Link
                                                        ...
                                                        className="btn"
                                                        to={"/patients/modify/" + this.props.match.params.id}
                                                    >
                                        ...
                                        <Row>
                                            ...
                                            <Col xs="6">
                                                ...
                                                {this.state.isEdit ? <button
                                                    ...
                                                    style={{ position: "absolute", top: '0', right: "8px", display: [1, 2, 3, 4].includes(diseases?.diseaseCategoryId) ? "block" : "none" }}
                                                    className="card-header-action btn btn-close"
                                                    to="#"
                                                    onClick={() => this.handleShowDisease()}
                                                >
                                        ...
                                        <Row>
                                            <Col xs="12">
                                                <FormGroup>
                                                    ...
                                                    < hr />
                                                    {ReactHtmlParser(this?.state?.basicInfo?.patientAccessionMapping?.diseaseDesc || "<p></p>")}
                                                </FormGroup>
                                                {this.state.isEdit ? <button
                                                    ...
                                                    className="card-header-action btn btn-close"
                                                    to="#"
                                                    onClick={() => this.handleShowDiseaseDescription()}
                                                >
                                                    <i className="icon-pencil"></i>
                                                </button> : ""}
                                            </Col>
                                    ...
                                    <Row>
                                        <Col xs="12">
                                            <FormGroup>
                                                ...
                                                < hr />
                                                {diseases?.diseaseCategory != "" && diseases?.diseaseCategory != null ? (
                                                    <span>
                                                        {diseases?.parentDiseaseCategory !== '' ? (diseases?.parentDiseaseCategory + ` - `) : (null)}
                                                        {diseases?.diseaseCategory}<b style={{ color: "red" }}>{diseases?.isMetastasis ? " - Metastasis" : ""}</b>
                                                    </span>
                                                ) : (
                                                    "Not Assigned"
                                                )}
                                        ...
                                        <Row>
                                            ...
                                            <Col xs="3">
                                                <FormGroup>
                                                    ...
                                                    >{assignedpractitioner.mobile || "Not Provided"}</a>
                                                </FormGroup>
                                            </Col>
                                            <Col xs="3">
                                                <FormGroup>
                                                    ...
                                                    >{assignedpractitioner.phoneNumber || "Not Provided"}</a>
                                                </FormGroup>
                                            </Col>
                                            {/* <Col xs="3">
                          <FormGroup>
                                                    ...
                                                    ? (
                                                        <div style={{ position: "absolute", right: "12px" }}>
                                                            <Link
                                                                ...
                                                                onClick={() => {
                                                                    this.setState({ practitionerId: assignedpractitioner.practitionerId })
                            ...
                            <Card className="viewPatientForm">
                                    ...
                                    <Row>
                                        <Col xs="12">
                                            ...
                                            <p className="mt-2 d-flex align-items-center justify-content-between">
                                                ...
                                                <Button
                                                    ...
                                                    color="primary"
                                                    Style={{ cursor: "pointer" }}
                                                    className="btn btn-primary text-light float-right" onClick={this.handleShowNotes}>
                                                            ...
                                                            <Col xs="12">
                                                                {
                                                                    ...
                                                                    }).length > 0 ? <p className="mt-2 d-flex align-items-center justify-content-between">
                                                                        <span className="h5 text-primary">

                                                                            {Pdata.sectionName}</span> {" "}
                                                                    </p> : null
                                                                }
                                                                ...
                                                                <Row>
                                                                    {
                                                                        this.state.AllpatientNotes.filter((Fdata) => {
                                                                            ...
                                                                            this.state.AllpatientNotes.filter((Fdata) => {
                                                                                ...
                                                                                return <Col xs="12" className="col-lg-4">
                                                                                    <div
                                        ...
                                        ></textarea> */}
                                                                ...
                                                                <b>
                                                                  Created at :
                                                                                                                    ...
                                                                                                                    <div className="d-flex align-items-center fle-column">
                                                                                                                        ...
                                                                                                                        <ReactTooltip
                                                                                                                            anchorId={Cdata.patientNoteId}
                                                                                                                            place="right"
                                                                                                                            variant="info"
                                                                                                                            content={Cdata.createdByUserName}

                                                                                                                        />
                                                                                                                        <div id={Cdata.patientNoteId} style={{ height: "32px", width: "32px" }}
                                                                                                                            className="bg-primary text-center d-flex justify-content-center rounded-pill border border-1">
                                                                                                                            <span className="mt-1 text-light">
                                                                                                                                <span>
                                                                                                                                    <small>
                                                                                                                            ...
                                                                                                                            >
                                                                                                                                {(confirm) => (
                                                                                                                                    <Button
                                                                                                                                        ...
                                                                                                                                        onClick={confirm((e) =>
                                                                                                                                            this.deletePatientNote(e, Cdata.patientNoteId)
                                                                                                                                        )}
                                                                                                                    ...
                                                                                                                    <span className="ml-2 float-right">
                                                                                                                        <small>
                                                                                                                            <b>
                                                                                                                                {
                                                                                                                                    function () {
                                                                                                                                        let a = Moment(Cdata.createdDate).format("Do MMM YYYY")
                                                                                                                                        // console.log("a", a)
                                                                                                                                        return <>{a.length > 12 ? a.slice(0, 2) : a.slice(0, 1)}<sup>{a.length > 12 ? a.slice(2, 4) : a.slice(1, 3)}</sup>{a.length > 12 ? a.slice(4, 8) : a.slice(3, 7)}{","}{a.length > 12 ? a.slice(8) : a.slice(7)}</>
                                                                                                                                    }()
                                           ...
                                           className="ml-1 note__save text-dark">View</button> */}
                                            ...
                                            <Modal isOpen={this.state.add_notes} className="modal-dialog modal-lg">
                                                <ModalBody className="">
                                                    <p className=""
                                                        ref={this.modal_notes_scroll_}
                                                    >
                                                        <span className="h4">
                                                            Create Note
                                                        </span>
                                                        ...
                                                        <FormGroup>
                                                            ...
                                                            >
                                                                ...
                                                                {this.state.sectionsNames.map((data, i) => {
                                                                    return (
                                                                        <option className="text-dark" key={i} value={data.id}>
                                                                            {data.sectionName}
                                                                        </option>
                                                                    );
                                                                })}
                              ...
                              <Button color="primary" disabled onClick={this.AddNotes.bind(this)}>
                                Add
                                                    ...
                                                    <Button color="primary" onClick={this.AddNotes.bind(this)}>
                                                        Create
                                                    </Button>
                                                    {/* } */}
                                                </ModalFooter>
                                            ...
                                            <Modal isOpen={this.state.edit_notes} className="modal-dialog modal-lg">
                                                <ModalBody>
                                                    <p className="">
                                                        <span className="h3">
                                                            Note
                            ...
                            >
                              ...
                              this.state.notes_updatedBy_Id == this.state.notes_createdBy_Id ? */}
                                                    ...
                                                    <Button color="primary"
                                                        disabled={
                                                            this.state.inputTextforNotes_before_Edit == this.state.inputTextforNotes ||
                                                            this.state.inputTextforNotes.trim() == ""
                                                        }
                                            ...
                                            {this.state.DeletemodalforNotes &&
                                                <div style={{ marginLeft: "36%" }}>
                                                    <Modal isOpen={this.state.DeletemodalforNotes} style={{ width: "500px" }} >
                                                        <ModalHeader>Confirm</ModalHeader>
                                                        <ModalBody>Are you sure you want to delete this comment?</ModalBody>
                                                        <ModalFooter>
                                                            <Button outline color="danger" onClick={e => this.deleteRowNote(e, this.state.id)}>Delete</Button>
                                            ...
                                            {this.state.DeletemodalforNoteComment &&
                                                <div style={{ marginLeft: "36%" }}>
                                                    <Modal isOpen={this.state.DeletemodalforNoteComment} style={{ width: "500px" }} >
                                                        <ModalHeader>Confirm</ModalHeader>
                                                        <ModalBody>Are you sure you want to delete this comment?</ModalBody>
                                                        <ModalFooter>
                                                            <Button outline color="danger" onClick={e => this.DeleteComment(e, this.state.DeleteCommentId, this.state.DeletepatientNotesId)}>Delete</Button>
                            ...
                            <Card className="viewPatientForm">
                                <CardBody>
                                    <Row>
                                        <Col xs="12">
                                            <h5 className="mt-2 w-100">Please Submit These Specimens</h5>
                                            < hr className="w-100" />
                                        </Col>
                            ...
                            >
                                <CardBody>
                                    <Row>
                                        <div className="form-group w-100 dis-contents">
                                            <Col xs="12" >
                                                <h5 className="mt-2 w-100">Analysis Payment</h5>
                                                < hr className="w-100" />
                                                    ...
                                                    {this?.state?.basicInfo?.showAnalysisOrder ? (
                                                        <FormGroup check row>
                                                            <Row style={{ marginBottom: "3px" }}>
                                                                ...
                                                                <Col xs="3" lg="3" xl="3" style={{ paddingTop: "5px" }}>
                                                                    <div className={`cus-date-picker `}>
                                                                        <DatePicker
                                                                            defaultValue=""
                                                                ...
                                                                <Col xs="4" lg="4" xl="4" style={{ paddingTop: "5px" }}>
                                      ...
                                      >Update</button> : */}
                                                                    <button
                                                                        ...
                                                                        style={{ paddingLeft: "3vw", paddingRight: "3vw", display: this.state.isEdit ? "block" : "none" }}
                                                                        disabled={
                                                                ...
                                                                >
                                                                    <Input
                                                                        ...
                                                                        disabled={!this.state.basicInfo?.analysisOrderStatus?.orderSubmited || this.state?.basicInfo?.analysisOrderStatus?.invoiceSend}
                                                                    />
                                                                </Col>
                                                                <Col
                                                                    xs="4" lg="4" xl="4"
                                                                ...
                                                                >
                                                                    <button
                                                                        ...
                                                                        style={{ paddingLeft: "3vw", paddingRight: "3vw", display: this.state.isEdit ? "block" : "none" }}
                                                                        disabled={
                                                                            this.isAnlysisPoDisabled() || this.state.analysisOrderStatus.poReceived.date === ""
                                                                            // !this.state.analysisOrderStatus.poReceived
                                                                            //   .checked ||
                                                                            // this.state.analysisOrderStatus.poReceived
                                                                    ...
                                                                    >
                                                                        {/* Submit */}
                                                                        <i className="fa fa-dot-circle-o text-light"></i> {this.state.basicInfo.analysisOrderStatus?.poReceived && !this.state.basicInfo.analysisOrderStatus?.invoiceSend ? "Update" : "Submit"}
                                                                    </button>
                                                                </Col>
                                                                <Col
                                                                    xs="1"
                                                                ...
                                                                >
                                                                    <Input
                                                                        ...
                                                                        disabled={!this.state?.basicInfo?.analysisOrderStatus?.poReceived || this.state.basicInfo?.analysisOrderStatus?.paymentReceived}
                                                                        onChange={() => {
                                                                            this.handleAnalysisOrderChange(
                                                                                "invoiceSent"
                                                                            );
                                                                ...
                                                                >
                                                                    <button
                                                                        ...
                                                                        style={{ paddingLeft: "3vw", paddingRight: "3vw", display: this.state.isEdit ? "block" : "none" }}
                                                                        disabled={
                                                                            this.isAnalysisInvoiceDisabled() || this.state.analysisOrderStatus.invoiceSent.date === ""
                                                                            // !this.state.analysisOrderStatus
                                                                            //   .invoiceSent.checked ||
                                                                            // this.state.analysisOrderStatus.invoiceSent
                                                                    ...
                                                                    >
                                                                        {/* Submit */}
                                                                        <i className="fa fa-dot-circle-o text-light"></i> {this.state.basicInfo.analysisOrderStatus?.invoiceSend && !this.state.basicInfo.analysisOrderStatus?.paymentReceived ? "Update" : "Submit"}
                                                                    </button>
                                                                </Col>
                                                                ...
                                                                >
                                                                    <Input
                                                                        ...
                                                                        disabled={!this.state.basicInfo?.analysisOrderStatus?.invoiceSend || this.state.basicInfo?.analysisOrderStatus?.paymentReceived}
                                                                        onChange={() => {
                                                                            this.handleAnalysisOrderChange(
                                                                                "paymentRecived"
                                                                            )
                                                                ...
                                                                >
                                                                    ...
                                                                    >
                                                                        {isFullPaymentStatus ? "Full" : ""} Payment Received <br />
                                                                        {isFullPaymentStatus ? <span style={{ color: "#000", fontSize: "12px" }}> for analysis and design</span> : ""}
                                                                    </Label>
                                                                </Col>
                                                                <Col
                                                                    xs="3" lg="3" xl="3"
                                                                ...
                                                                >
                                                                    <div className={`cus-date-picker `}>
                                                                        <DatePicker
                                                                            ...
                                                                            disabled={
                                                                                !this.state.analysisOrderStatus
                                                                                    .paymentRecived.checked || !this?.state?.basicInfo?.analysisOrderStatus?.invoiceSend || this.state.basicInfo
                                                                                        .analysisOrderStatus?.paymentReceived
                                                                            }
                                                                            dateFormat="MM/dd/yyyy"
                                                                            placeholderText="mm/dd/yyyy"
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        disabled={
                                                                            !this.state.analysisOrderStatus
                                                                                .paymentRecived.checked ||
                                                                            this.state.analysisOrderStatus
                                                                                .paymentRecived.date === "" || !this?.state?.basicInfo?.analysisOrderStatus?.invoiceSend || this.state.basicInfo
                                                                                    .analysisOrderStatus?.paymentReceived
                                                                        }
                                                                        onClick={() => {
                                                                            this.updateDataAndRefreshPage(() => {
                                  ...
                                  >
                                    Submit
                                                        ...
                                                        <>
                                                            <FormGroup >
                                                                After assign specimen collection, this section will be visible.
                                                            </FormGroup>
                                                        </>
                                                    )}
                                                </FormGroup>
                                                    ...
                                                    <Col xs="9" lg="10" xl="9">
                                                        <div className="form-group w-100 dis-contents">
                                                            <FormGroup>
                                                                ...
                                                                <FormGroup check row>
                                                                    <Row className="d-flex justify-content-start align-items-center" style={{ marginBottom: "3px" }}>
                                                                        ...
                                                                        {filteredSpecimenReceivedStatus.map(
                                                                            (child) => {
                                                                                ...
                                                                                return (
                                                                                    <>
                                                                                        <Col xs="1" lg="1" xl="1" className="align-self-start my-2">
                                                                                            {/* {chyild.sampleTypeId} */}
                                                                                            <Input
                                                                                                className={`ml-0  ${isEditDateClass}  `}
                                                                                                type="checkbox"
                                                                                        ...
                                                                                        <Col xs="3" className="align-self-start my-2" lg="3" xl="3">
                                                                                            <div
                                                                                        ...
                                                                                        <Col className=" my-2" id="SpecimenSectionNotApplicable" xs="1" lg="1" xl="1">
                                                                                            <Input
                                                                                                className={`ml-0 mt-1  ${isEditDateClass} mr-3 `}
                                                                                                type="checkbox"
                                                                                                name="inline-checkbox1"
                                                                                                disabled={
                                                                                                    this.disablesNaData(ele.ngsLaboratoryId,
                                                                                                        child.ngsLaboratoryPatientActivityId)
                                                                                                }
                                                                                        ...
                                                                                        <Col className="align-self-start my-2" xs="3" lg="3" xl="3">
                                                                                            {this.displayApplyDate(ele.ngsLaboratoryId, child.ngsLaboratoryPatientActivityId, (labId, child) => this.isDateDisabledForSelectedLabs(labId, child) || this.diasbledCheckboxForSpecimenCollectionSampleLabs(labId, child))}
                                                                                        </Col>
                                                                                    </>
                                                                                );
                                                                            }
                                                                        ...
                                                                        {filteredSpecimenReceivedStatus.length ?
                                                                            <Col xs="8" className="text-right  " style={{ marginLeft: '4.5vw', display: "flex", justifyContent: "end" }}>
                                                                                <button
                                                                                    style={{ paddingLeft: '3vw', paddingRight: "3vw", display: this.state.isEdit ? "block" : "none" }}
                                                                                    ...
                                                                                    disabled={
                                                                                        ...
                                                                                        this.isSubmitDisabledForSampleLab(ele.ngsLaboratoryId, "selectedLabs")
                                                                                ...
                                                                                >
                                                                                    <i className="fa fa-dot-circle-o text-light"></i> Submit
                                                                                </button></Col> : <FormGroup className="ml-3">This laboratory is not working on assigned samples.</FormGroup>}
                                                                    </Row>
                                                                        ...
                                                                        <FormGroup check row>
                                                                            <Row style={{ marginBottom: "3px" }}>
                                                                                    ...
                                                                                    (child) => {
                                                                                        ...
                                                                                        return (
                                                                                            <>
                                                                                                ...
                                                                                                <Col xs="4" lg="4" xl="4" className="align-self-start my-2"
                                                                                                ...
                                                                                                <Col xs="1" lg="1" xl="1" className="align-self-start my-2">
                                                                                                    {child?.analysisNotApplicable ? <Label
                                                                                                ...
                                                                                                <Col xs="3" lg="3" xl="3" className="align-self-start my-2">
                                                                                                    {this.displayApplyDateForBucket(ele.ngsLaboratoryId, child.ngsLaboratoryPatientActivityId, (labId, child) => this.isDateDisabledForSelectedLabsOfBucket(labId, child) || this.isSampleUploadedToBucket(labId, child))}
                                                                                                </Col>
                                                                                ...
                                                                                {filteredArrOfLabRecievedResult?.length ?
                                                                                    <Col xs="8" style={{ marginLeft: '4.5vw' }}>  <button
                                                                                        style={{ paddingLeft: '3vw', paddingRight: "3vw", display: this.state.isEdit ? "block" : "none" }}

                                                                                        className="btn btn-primary btn-md mt-2 float-right"
                                                                                        // style={{margin: "10px auto"}}
                                                                                        disabled={
                                                                                            this.isSubmitDisabledForSampleLab(ele.ngsLaboratoryId, "selectedLabsForBucket")
                                                                                    ...
                                                                                    >
                                                                                        <i className="fa fa-dot-circle-o text-light"></i> Submit
                                                                                    </button></Col> : "This laboratory is not working on assigned samples."}
                                                                            </Row>
                                                                        </FormGroup> : <FormGroup>
                                                                            Please send one of the samples to view result</FormGroup>}
                                                            </FormGroup>
                                                        </div>
                                                    </Col>
                                            ...
                                            <div className="form-group w-100 dis-contents">
                                                <Col xs="6">
                                                    <FormGroup>
                                                        ...
                                                        <hr />
                                                        {/* <h6 className="mt-2"> */}
                                                        This section will be visible after analysis order confirmed.{" "}
                                                        {/* </h6> */}
                                                    </FormGroup>
                                                </Col>
                                            </div>
                            ...
                            <Card className="viewPatientForm">
                                <CardBody>
                                    <Row>
                                        ...
                                        <div className="form-group w-100 dis-contents">
                                            <Col xs="12">
                                                <h5 className="mt-2 w-100">Design Payment </h5>
                                                < hr className="w-100" />
                                            ...
                                            <Col xs="8" lg="9" xl="7">
                                                <FormGroup>
                                                    ...
                                                    {this.state.basicInfo.showDesignOrder ? (
                                                            ...
                                                            <Row style={{ marginBottom: "3px" }}>
                                                                <Col xs="1" className="align-self-start my-2">
                                                                    <Input
                                                                        ...
                                                                        onChange={() => {
                                                                            this.handleDesignOrderStatusChange("orderSubmited");
                                                                        }}
                                                                        checked={this.isCheckedForDesignOrderStatus("orderSubmited")}
                                                                        // disabled={true}
                                                                ...
                                                                <Col xs="4" className="align-self-start my-2" >
                                                                    <Label className="form-check-label" check htmlFor='do_check_order_submit'>
                                                                        {/* Order Submitted */}
                                                                        Send PO Notification
                                                                    </Label>
                                                                </Col>
                                                                <Col xs="3" className="align-self-start my-2" style={{ paddingTop: "5px" }}>
                                                                    <div className={`cus-date-picker `}>
                                                                        <DatePicker
                                                                ...
                                                                <Col xs="4" className="align-self-start my-2" style={{ paddingTop: "5px" }}>
                                                                    <button
                                                                        ...
                                                                        disabled={
                                                                            !this.getDateForDesignOrderStatus(
                                                                                "orderSubmited"
                                                                            ) || this.isDesignOrderSubmittedDisabled()
                                                                        }
                                                                        onClick={() => {
                                                                            this.updateDataAndRefreshPage(() => {
                                                                ...
                                                                <Col xs="1" className="align-self-start my-2">
                                                                    <Input
                                                                        ...
                                                                        checked={this.isCheckedForDesignOrderStatus("poReceived")}
                                                                        className={`ml-0  ${isEditDateClass}  `}
                                                                        type="checkbox"
                                                                        name="inline-checkbox1"
                                                                ...
                                                                <Col xs="4" className="align-self-start my-2" >
                                                                    <Label className="form-check-label" htmlFor="chkbox_designOrdrStat_po">
                                                                        PO Sent to Accounting
                                                                    </Label>
                                                                </Col>
                                                                <Col xs="3" className="align-self-start my-2" style={{ paddingTop: "5px" }}>
                                                                    <div className={`cus-date-picker `}>
                                                                        <DatePicker
                                                                            ...
                                                                            selected={this.getDateForDesignOrderStatus("poReceived")}
                                                                            defaultValue=""
                                                                            dateFormat="MM/dd/yyyy"
                                                                            placeholderText="mm/dd/yyyy"
                                                                            className={`form-control here text-center  ${isEditDateClass}  `}
                                                                ...
                                                                <Col xs="4" className="align-self-start my-2" style={{ paddingTop: "5px" }}>
                                                                    <button
                                                                        className={`btn btn-${this.state.basicInfo?.designOrderStatus?.poReceived && !this.state.basicInfo?.designOrderStatus?.invoiceSend ? "success" : "primary"} btn-md `}
                                                                        style={{ paddingLeft: "3vw", paddingRight: "3vw", display: this.state.isEdit ? "block" : "none" }}
                                                                        disabled={
                                                                            !this.getDateForDesignOrderStatus(
                                                                                "poReceived"
                                                                            ) || this.isDesignOrderPoDisabled()
                                                                        }
                                                                        onClick={() => {
                                                                            this.updateDataAndRefreshPage(() => {
                                                                    ...
                                                                    >
                                                                        {/* Submit */}
                                                                        <i className="fa fa-dot-circle-o text-light"></i> {this.state.basicInfo?.designOrderStatus?.poReceived && !this.state.basicInfo?.designOrderStatus?.invoiceSend ? "Update" : "Submit"}
                                                                    </button>
                                                                </Col>
                                                                {/* PO Received By Acct */}
                                                                {/* Invoice Sent to Clinic */}
                                                                ...
                                                                <Col xs="1" className="align-self-start my-2" >
                                                                    <Input
                                                                        ...
                                                                        checked={this.isCheckedForDesignOrderStatus("invoice")}
                                                                        onChange={() => {
                                                                        ...
                                                                        disabled={!this.state.basicInfo?.designOrderStatus?.poReceived || this.state.basicInfo?.designOrderStatus?.paymentReceived}

                                                                    />
                                                                </Col>
                                                                <Col xs="4" className="align-self-start my-2">
                                                                    <Label className="form-check-label" htmlFor="chkbox_designOrdrStat_invoice">
                                                                        Invoice Sent to Clinic
                                                                    </Label>
                                                                </Col>
                                                                <Col xs="3" className="align-self-start my-2" style={{ paddingTop: "5px" }}>
                                                                ...
                                                                <Col xs="4" className="align-self-start my-2" style={{ paddingTop: "5px" }}>
                                                                    <button
                                                                        className={`btn btn-${this.state.basicInfo?.designOrderStatus?.invoiceSend && !this.state.basicInfo?.designOrderStatus?.paymentReceived ? "success" : "primary"} btn-md `}
                                                                        style={{ paddingLeft: "3vw", paddingRight: "3vw", display: this.state.isEdit ? "block" : "none" }}
                                                                        disabled={
                                                                            !this.getDateForDesignOrderStatus(
                                                                                "invoice"
                                                                            ) || this.isDesignOrderInvoiceDisabled()
                                                                        }
                                                                        onClick={() => {
                                                                            this.updateDataAndRefreshPage(() => {
                                                                ...
                                                                <Col xs="1" className="align-self-start my-2">
                                                                    <Input
                                                                        ...
                                                                        checked={this.isCheckedForDesignOrderStatus("paymentRecived")}
                                                                        className={`ml-0  ${isEditDateClass}  `}
                                                                        disabled={this.isDesignOrderStatusDisabled('paymentReceived') || !this.isDesignOrderStatusDisabled('invoiceSend')}
                                                                        type="checkbox"
                                                                        name="inline-checkbox1"
                                                                ...
                                                                <Col xs="4" className="align-self-start my-2" >
                                                                    <Label className="form-check-label" htmlFor="chkbox_designOrdrStat_payment">
                                                                        Payment Received <br />
                                                        ...
                                                        <>
                                                            <FormGroup>
                                                                This section will be visible after receiving the data.
                                                            </FormGroup>
                                                        </>
                                                    )}
                                                </FormGroup>
                                            ...
                                            <FormGroup>
                                                ...
                                                {this.state.basicInfo?.showDesignActivity ? <>
                                                    ...
                                                    >
                                                        ...
                                                        >Pipeline </h5>
                                                        <Row className="ml-1 text-center justify-content-center"
                                                            style={{ display: [7, 8, 9, 10].includes(this.state?.diseases?.diseaseCategoryId) ? "" : "none" }}

                                                        >
                                                            This section is under development for LRI analysis
                                                        </Row>
                                                        <Row className="ml-1 text-center justify-content-center"
                                                            style={{ display: [6, 11, 12].includes(this.state?.diseases?.diseaseCategoryId) ? "" : "none" }}
                                                        ...
                                                        <Row className="ml-1"
                                                            style={{ display: [6, 7, 8, 9, 10, 11, 12].includes(this.state?.diseases?.diseaseCategoryId) ? "none" : "" }}

                                                        >
                                                            {/* {!this.state.basicInfo?.designActivity?.designStarted && */}
                                                            <button
                                                                id={[6, 7, 8, 9, 10, 11, 12].includes(this.state?.diseases?.diseaseCategoryId) ? "" : "PMRandFinalReportVerifyData"}
                                                                ...
                                                                disabled={this.isAllLabSamplesAssignedForVerify()
                                                        ...
                                                        >
                                                            ...
                                                            <button
                                                                ...
                                                                style={{ maxWidth: "170px", height: "42px" }}
                                                                id={[6, 7, 8, 9, 10, 11, 12].includes(this.state?.diseases?.diseaseCategoryId) ? "" : "PMRandFinalReportProcessData"}
                                                                ...
                                                                // }
                                                                disabled={this.isAllLabSamplesAssignedForProcess()  // main
                                                                ...
                                                                onClick={() => {
                                                                    if (this.state.basicInfo?.designActivity?.designFailed == true || this.state.basicInfo?.designActivity?.designWarning == true || this.state.basicInfo?.designActivity?.designCompleted == true) {
                                                                        if ([1, 2, 3, 4].includes(diseases?.diseaseCategoryId)) {
                              ...
                              <>
                                ...
                                {true && <> */}
                                                        ...
                                                        <Col xs="12">
                                                            <div className="text-center">
                                                                ...
                                                                <>
                                                                    {Array.isArray(this.state.basicInfo.patientDisease) &&
                                                                        this.state.basicInfo.patientDisease.length > 0 &&
                                                                        this.state.basicInfo.patientDisease.map((ele) => {
                                                                            return <>
                                                                                <button
                                                                                    style={{ display: [2, 3, 1, 5].includes(ele.diseaseCategoryId) ? "inline-block" : "none" }}
                                                                                    ...
                                                                                    onClick={() => {
                                                                                        ...
                                                                                        if ((this.state?.diseaseDataId == 0 || this.state?.diseaseDataId == null) && [1, 2, 3].includes(this.state?.diseases?.diseaseCategoryId)) {
                                                                                            this.handleShowDisease(false, false, true)
                                                                                        } else {
                                                                                            this.props.history.push({
                                                                                                pathname: `/patientactivity/designactivities/report/${this.state.patientId}/${this.state.PatientAccessionId}/`,
                                                                                    ...
                                                                                    id={[2, 3, 1, 5].includes(ele.diseaseCategoryId) ? "PMRandFinalReportGPR" : ""}
                                                                                ...
                                                                                <button
                                                                                    className="btn btn-success btn-md m-2 "
                                                                                    style={{ display: [6, 11, 12].includes(ele.diseaseCategoryId) ? "inline-block" : "none" }}
                                                                                    ...
                                                                                    disabled={([6, 11, 12].includes(this.state?.diseases?.diseaseCategoryId) && !this.state.basicInfo?.designActivity?.designCompleted)}
                                                                                    onClick={() => {
                                                                                        this.props.history.push({
                                                                                    ...
                                                                                    id={![6, 11, 12].includes(ele.diseaseCategoryId) ? "" : "PMRandFinalReportGPreventionR"}

                                                                                // disabled={'Prevention' || 'Complete Health Score' || 'HealthIndex'}
                                                                                >
                                                                                    Generate Prevention Report </button>

                                                                                <button
                                                                                    className="btn btn-success btn-md m-2 "
                                                                                    style={{ display: [7, 8, 9, 10].includes(ele.diseaseCategoryId) ? "inline-block" : "none" }}
                                                                                    disabled={true}
                                                                                    // disabled={this.state?.basicInfo?.patientReports?.healthIndexReport || ([2, 3, 4].includes(this.state?.diseases?.diseaseCategoryId) && !this.state.basicInfo?.designActivity?.designCompleted)}
                                                                                    // onClick={() => {
                                                                                    //   this.props.history.push({
                                                                                    ...
                                                                                    // }}
                                                                                    // disabled={'Prevention' || 'Complete Health Score' || 'HealthIndex'}
                                                                                    id={[7, 8, 9, 10].includes(ele.diseaseCategoryId) ? "PMRandFinalReportGLongevityR" : ""}
                                                                                >
                                                                                    Generate Longevity Report </button>
                                                                                <button
                                                                                    ...
                                                                                    id={[7, 8, 9, 10].includes(ele.diseaseCategoryId) ? "PMRandFinalReportGPMR" : ""}
                                                                                ...
                                                                                >
                                                                                    {this.state?.basicInfo?.patientReports?.pmrReport ? "PMR Report Generated" : 'Generate PMR Report'}
                                                                                </button>
                                                                                <button
                                                                                    // style={{ display: [5, 7, 8, 9, 10].includes(ele.diseaseCategoryId) ? "none" : "inline-block" }}
                                                                                    style={{ display: [7, 8, 9, 10].includes(ele.diseaseCategoryId) ? "none" : "inline-block" }}
                                                                                ...
                                                                                {true && <button
                                                                                    ...
                                                                                    onClick={
                                                                                        this.admeToxSaveData.bind(this)
                                                                                    }
                                                                                    disabled={(!this.state.basicInfo?.designActivity?.designCompleted && !this.state.basicInfo?.designActivity?.designWarning)}


                                                                                    // disabled={'Prevention' || 'Complete Health Score' || 'HealthIndex'}
                                                                                    id="PMRandFinalReportGAToxR"
                                                                                >
                                                                                    Generate ADME-Tox Report </button>}
                                                                        ...
                                                                        <tbody className="m-2">
                                                                            {this.state.basicInfo?.patientReports?.patientReport &&
                                                                                <tr className="px-2 d-flex justify-content-between  align-items-center ">
                                                                                    ...
                                                                                    >
                                                                                        <i className="fa fa-eye text-light opacity-50" aria-hidden="true"></i><span></span></button></td>
                                                                                </tr>
                                                                            }
                                                                            ...
                                                                            {this.state.basicInfo?.patientReports?.pmrReport &&
                                                                                <tr className="px-2 d-flex justify-content-between align-items-center " >
                                                                                    ...
                                                                                    <td className="pr-4">
                                                                                        <button
                                                                                            className="btn btn-success float-right "
                                                                                            data-toggle="tooltip" data-html="true" data-placement="right" title="View PMR Report"
                                                                                            onClick={() => {
                                                                                                this.previewPdfToggle(this.state.basicInfo.patientReports.pmrReport)
                                                                                            }}
                                                                                        >  <i className="fa fa-eye text-light opacity-50" aria-hidden="true"></i><span></span>
                                                                                        </button>
                                                                                    </td>
                                                                                    -
                                                                                </tr>
                                                                            }
                                                                            {this.state.basicInfo?.patientReports?.healthIndexReport &&
                                                                                <tr className="px-2 d-flex justify-content-between align-items-center ">
                                                                                    {/* <th scope="row"><i class="cil-document"></i>NB2100000_ITI_PES_Final_Report</th> */}
                                                                                    <td style={{ verticalAlign: "inherit", display: "flex", alignItems: "center" }}>
                                                                                    ...
                                                                                    <td className="pr-4"><button
                                                                                        className="btn btn-success float-right "
                                                                                        data-toggle="tooltip" data-html="true" data-placement="right" title="View Health Index Report"
                                                                                        onClick={() => {
                                  ...
                                  >{this.state.basicInfo?.patientReports?.isPMRApproved ? "PMR Approved" : "Approve PMR"}</button>
                                                        ...
                                                        {[2, 3, 1, 5].includes(this.state?.diseases?.diseaseCategoryId) ?
                                                            <Col xs="12">

                                                                <table className="table table-bordered  ">
                                                                    <thead style={{ color: "#fff", border: "1px solid #FFF" }}>
                                                                            ...
                                                                            <th className="border-0" colspan="3" >
                                                                                <div className="d-flex" style={{ justifyContent: "space-between", alignItems: "center" }}>
                                                                                    ...
                                                                                    <button className=" btn btn-secondary "
                                                                                        ...
                                                                                        style={{ fontWeight: "700" }} onClick={() => { this.handleShowUploadFile("Patient Report", 1) }}>Upload </button>
                                                                                </div>
                                                                            </th>
                                                                        </tr>
                                                                                ...
                                                                                <table className="table table-bordered ">
                                                                                    <thead>
                                                                                        <tr className="text-center" style={{
                                                                                            ...
                                                                                            backgroundColor: "#e4e7ea", color: "black"
                                                  ...
                                                  Version
                                                                                    ...
                                                                                    <tbody>
                                                                                        {Array.isArray(this.state.patientReports) && this.state.patientReports?.length > 0 ?
                                                                                            this.state.patientReports.map((e, i) => {

                                                                                                return <tr>
                                                                                                    <td className="w-50" style={{ verticalAlign: "inherit", alignItems: "center" }}>
                                                                                                        <i className="fa fa-file-pdf-o" style={{ margin: "5px", color: "#f64846" }}></i>
                                                                                                        {/* {e.patientReport.slice(e.patientReport.lastIndexOf("/"), e.patientReport.length )} */}

                                                                                                        {this.state.renamePdf === ("pr" + i) ? <div style={{ float: "right", width: "92%", position: "relative", textAlign: "center" }}>
                                                                                                            <Input type="text" onChange={(e) => { this.handlePdfNameChange(e) }} value={this.state?.filePdfname} />
                                                                                                            <i
                                                                                                                style={{ position: "absolute", right: "9px", top: "11px", cursor: "pointer" }}
                                                                                                                onClick={() => { this.setState({ renamePdf: "", filePdfname: "" }) }}
                                                                                                                class="fa fa-close">
                                                                                                            </i>
                                                                                                            <button className="btn btn-success mt-1"
                                                                                                                onClick={event => this.RenameFun(event, e.id, 1)}
                                                                                                            ...
                                                                                                            onDoubleClick={() => { this.setState({ renamePdf: "pr" + i, filePdfname: this.spliceFromSash(e.patientReport).replace(".pdf", "") }) }}
                                                                                                        >{this.spliceFromSash(e.patientReport)}</span>}
                                                                                                    ...
                                                                                                    <td> <button type="btn" className="btn  btn-success btn-pill" onClick={() => this.previewPdfToggle_api(e.id, 1)}>View</button> <button type="btn" className="btn  btn-danger btn-pill" onClick={() => { this.handleShowDeleteFile(e.id) }}>Delete</button></td>
                                                                                                </tr>
                                                                                            })
                                                                                            : <tr><td colspan="3" className="text-center">Generated reports will be displayed here.</td></tr>}
                                        ...
                                        <td> <button type="btn" className="btn  btn-danger btn-pill">Delete</button> <button type="btn" className="btn  btn-success btn-pill">View</button></td>
                                                        ...
                                                        {[6, 11, 12].includes(this.state?.diseases?.diseaseCategoryId) ?
                                                            <Col xs="12">
                                                                <table className="table table-bordered  ">
                                                                    <thead style={{ color: "#fff", border: "1px solid #FFF" }}>
                                                                        <tr style={{ backgroundColor: "#1C3A84", }}>
                                                                            <th className="border-0" colspan="3" >
                                                                                <div className="d-flex" style={{ justifyContent: "space-between", alignItems: "center" }}>
                                                                                    ...
                                                                                    <button className=" btn btn-secondary "
                                                                                        //  disabled={!this.state.basicInfo?.designActivity?.designCompleted} 
                                                                                        style={{ fontWeight: "700" }} onClick={() => { this.handleShowUploadFile("Healthindex Report", 3) }}>Upload </button>
                                                                                </div>
                                                                            </th>
                                                                        </tr>
                                                  ...
                                                  Version
                                                                                    ...
                                                                                    <tbody>
                                                                                        {Array.isArray(this.state.patientHIReports) && this.state.patientHIReports?.length > 0 ?
                                                                                            this.state.patientHIReports.map((e, i) => {
                                                                                                // console.log(e);
                                                                                                return <tr>
                                                                                                    <td className="w-50" style={{ verticalAlign: "inherit", alignItems: "center" }}>
                                                                                                        <i className="fa fa-file-pdf-o" style={{ margin: "5px", color: "#f64846" }}></i>
                                                                                                        {/* {e.pmrReport} */}
                                                                                                        {this.state.renamePdf === ("pvr" + i) ? <div style={{ float: "right", width: "92%", position: "relative", textAlign: "center" }}>
                                                                                                            <Input type="text" onChange={(e) => { this.handlePdfNameChange(e) }} value={this.state?.filePdfname} />
                                                                                                            <i
                                                                                                                style={{ position: "absolute", right: "9px", top: "11px", cursor: "pointer" }}
                                                                                                                onClick={() => { this.setState({ renamePdf: "", filePdfname: "" }) }}
                                                                                                            ...
                                                                                                            <button className="btn btn-success mt-1"
                                                                                                                onClick={event => this.RenameFun(event, e.id, 3)}
                                                                                                            >Submit<i
                                                                                                                style={{ color: "#fff", marginLeft: "3px" }}
                                                                                                                class="fa fa-check">
                                                                                                                </i></button>
                                                                                                            ...
                                                                                                            onDoubleClick={() => { this.setState({ renamePdf: "pvr" + i, filePdfname: this.spliceFromSash(e.healthIndexReport).replace(".pdf", "") }) }}
                                                                                                        >{this.spliceFromSash(e.healthIndexReport)}</span>}
                                                                                                    ...
                                                                                                    <td style={{ "display": "flex", "alignItems": "center", "justifyContent": "center", "gap": "10px" }}>
                                                                                                        <button type="btn" className="btn  btn-success btn-pill" onClick={() => this.previewPdfToggle_api(e.id, 3)}>View</button>
                                        ...
                                        <tr>
                                          ...
                                          <td> <button type="btn" className="btn  btn-success btn-pill">View</button> <button type="btn" className="btn  btn-danger btn-pill" onClick={() => { this.handleShowDeleteFile() }}>Delete</button> </td>
                                                        ...
                                                        {[7, 8, 9, 10].includes(this.state?.diseases?.diseaseCategoryId) ?
                                                            <Col xs="12">
                                                                                            ...
                                                                                            this.state.patientReports.map((e, i) => {
                                                                                                ...
                                                                                                return <tr>
                                                                                                    <td className="w-50" style={{ verticalAlign: "inherit", alignItems: "center" }}>
                                                                                                        <i className="fa fa-file-pdf-o" style={{ margin: "5px", color: "#f64846" }}></i>
                                                                                                        {/* {e.patientReport.slice(e.patientReport.lastIndexOf("/"), e.patientReport.length )} */}
                                                                                                        {this.state.renamePdf === ("lr" + i) ? <div style={{ float: "right", width: "92%", position: "relative", textAlign: "center" }}>
                                                                                                            <Input type="text" onChange={(e) => { this.handlePdfNameChange(e) }} value={this.state?.filePdfname} />
                                                                                                            <i
                                                                                                                style={{ position: "absolute", right: "9px", top: "11px", cursor: "pointer" }}
                                                                                                                onClick={() => { this.setState({ renamePdf: "" }) }}
                                                                                                                class="fa fa-close">
                                                                                                            </i>
                                                                                                            <button className="btn btn-success mt-1"
                                                                                                            // onClick={event => this.RenameFun(event, e.id, 2)}
                                                                                                            ...
                                                                                                            onDoubleClick={() => { this.setState({ renamePdf: "lr" + i, filePdfname: this.spliceFromSash(e.patientReport).replace(".pdf", "") }) }}
                                                                                                        >{this.spliceFromSash(e.patientReport)}</span>}
                                                                                                    ...
                                                                                                    {/* <td>{e.pmrCurrentVersion}</td> */}
                                                                                                    <td> <button type="btn" className="btn  btn-success btn-pill" onClick={() => this.previewPdfToggle(e.patientReport)}>View</button> <button type="btn" className="btn  btn-danger btn-pill" onClick={() => { this.handleShowDeleteFile(e.id) }}>Delete</button></td>
                                                                                                </tr>
                                                                                            })
                                                                                            : <tr><td colspan="3" className="text-center">Generated reports will be displayed here.</td></tr>}
                                        ...
                                        <td> <button type="btn" className="btn  btn-danger btn-pill">Delete</button> <button type="btn" className="btn  btn-success btn-pill">View</button></td>
                                                        ...
                                                        {false ? "" :
                                                            <Col xs="12">
                                                                <table className="table table-bordered  ">
                                                                    <thead style={{ color: "#fff", border: "1px solid #FFF" }}>
                                                                        <tr className="text-center" style={{ backgroundColor: "#1C3A84", }}>
                                                                            <th className="border-0" colspan="4" >
                                                                                <div className="d-flex" style={{ justifyContent: "space-between", alignItems: "center" }}>
                                                                                    ...
                                                                                    <button className="btn btn-secondary "
                                                                                        ...
                                                                                        // disabled={!this.state.basicInfo?.designActivity?.designCompleted}
                                                                                        style={{ fontWeight: "700" }} onClick={() => { this.handleShowUploadFile("PMR Report", 2) }}>Upload </button>
                                                                                </div>
                                                                            </th>
                                                                        </tr>
                                        ...
                                        Approve and Verify
                                                                                    ...
                                                                                    <tbody>
                                                                                        {Array.isArray(this.state.patientPMRReports) && this.state.patientPMRReports?.length > 0 ?
                                                                                            this.state.patientPMRReports.map((e, i) => {
                                                                                                // console.log(e)
                                                                                                return <tr>
                                                                                                    <td className="w-50" style={{ verticalAlign: "inherit", alignItems: "center" }}>
                                                                                                        <i className="fa fa-file-pdf-o" style={{ margin: "5px", color: "#f64846" }}></i>
                                                                                                        ...
                                                                                                        {this.state.renamePdf === ("pmr" + i) ? <div style={{ float: "right", width: "92%", position: "relative", textAlign: "center" }}>
                                                                                                            <Input type="text" onChange={(e) => { this.handlePdfNameChange(e) }} value={this.state?.filePdfname} />
                                                                                                            <i
                                                                                                                style={{ position: "absolute", right: "9px", top: "11px", cursor: "pointer" }}
                                                                                                                onClick={() => { this.setState({ renamePdf: "", filePdfname: "" }) }}
                                                                                                            ...
                                                                                                            onDoubleClick={() => { this.setState({ renamePdf: "pmr" + i, filePdfname: this.spliceFromSash(e.pmrReport).replace(".pdf", "") }) }}
                                                                                                        >{this.spliceFromSash(e.pmrReport)}</span>}
                                                      ...
                                                      <button type="btn" disabled={e.isPMRApproved} style={{ fontWeight: "700" }} className="btn  btn-primary btn-pill" onClick={() => { this.setState({ showPmrApprove: !this.state?.showPmrApprove, version: e.pmrCurrentVersion, verifyPmrPath: e.pmrReport, verifyId: e.id }) }}>{e.isPMRApproved ? "Approved" : "Approve"}</button>
                                                                                                    ...
                                                                                                    <td style={{ "display": "flex", "alignItems": "center", "justifyContent": "center", "gap": "10px" }}>
                                                                                                        <button type="btn" id="PMRandFinalReportPMRFileView" className="btn  btn-success btn-pill" onClick={() => this.previewPdfToggle_api(e.id, 2)}>View</button>
                                                                                                        <button type="btn" id="PMRandFinalReportPMRFileDelete" className="btn  btn-danger  btn-pill" onClick={() => { this.handleShowDeleteFile(e.id) }} >Delete</button>
                                                                                                        <button type="btn" id="PMRandFinalReportPMRFileVerify" disabled={e.isPMRVerified} style={{ fontWeight: "700" }} className="btn  btn-secondary btn-pill" onClick={() => { this.setState({ showPmrVerify: !this.state?.showPmrVerify, notes: e.notes || "<p><p/>", version: e.pmrCurrentVersion, verifyPmrPath: e.pmrReport, verifyId: e.id, PMRINDEX: i }) }}>{e.isPMRVerified ? "Verified" : "Verify"}</button>
                                                                                                        <button type="btn" id="PMRandFinalReportPMRFileApproved" disabled={!e.isPMRVerified || e.isPMRApproved || e.pmrRejectDate} style={{ backgroundColor: e?.pmrRejectDate && "red", border: e?.pmrRejectDate && "none", opacity: e.pmrRejectDate !== null && "0.5", fontWeight: "700" }} className="btn  btn-primary btn-pill" onClick={() => { this.setState({ showPmrApprove: !this.state?.showPmrApprove, version: e.pmrCurrentVersion, verifyPmrPath: e.pmrReport, verifyId: e.id }) }}>{e.pmrRejectDate
                                                                                                            ? "Rejected" : e.isPMRApproved ? "Approved" : "Approve"}</button>
                                                                                                    </td>
                                                                                                </tr>
                                                                                            }) : <tr><td colspan="4" className="text-center">Generated reports will be displayed here.</td></tr>}
                                      ...
                                      <td> <button type="btn" className="btn  btn-danger btn-pill">Delete</button> <button type="btn" className="btn  btn-success btn-pill">View</button></td>
                                                  ...
                                                  Version
                                                                                    ...
                                                                                    <tbody>
                                                                                        {Array.isArray(this.state.PatientAdmeToxReports) && this.state.PatientAdmeToxReports?.length > 0 ?
                                                                                            this.state.PatientAdmeToxReports.map((e, i) => {
                                                                                                return <tr>
                                                                                                    <td className="w-50" style={{ verticalAlign: "inherit", alignItems: "center" }}>
                                                                                                        <i className="fa fa-file-pdf-o" style={{ margin: "5px", color: "#f64846" }}></i>
                                                                                                        {this.state.renamePdf === ("adm" + i) ? <div style={{ float: "right", width: "92%", position: "relative", textAlign: "center" }}>
                                                                                                            <Input type="text" onChange={(e) => { this.handlePdfNameChange(e) }} value={this.state?.filePdfname} />
                                                                                                            <i
                                                                                                                style={{ position: "absolute", right: "9px", top: "11px", cursor: "pointer" }}
                                                                                                                onClick={() => { this.setState({ renamePdf: "", filePdfname: "" }) }}
                                                                                                            ...
                                                                                                            <button className="btn btn-success mt-1"
                                                                                                                ...
                                                                                                                onClick={event => this.RenameFun(event, e.id, 4)}
                                                                                                            ...
                                                                                                            onDoubleClick={() => { this.setState({ renamePdf: "adm" + i, filePdfname: this.spliceFromSash(e.admeToxReport).replace(".pdf", "") }) }}
                                                                                                        >{this.spliceFromSash(e.admeToxReport)}</span>}
                                                                                                    ...
                                                                                                    <td style={{ "display": "flex", "alignItems": "center", "justifyContent": "center", "gap": "10px" }}>
                                                                                                        <button type="btn" className="btn  btn-success btn-pill" onClick={() => this.previewPdfToggle_api(e.id, 4)}>View</button>
                                                                                                        <button type="btn" className="btn  btn-danger  btn-pill" onClick={() => { this.handleShowDeleteFile(e.id) }} >Delete</button>
                                                                                                    </td>
                                                                                                </tr>
                                                                                            })
                                                    ...
                                                    <FormGroup>This section will be visible after receiving the design payment.</FormGroup>}
                                            </FormGroup>
                                        </Col>
                                    </Row>
                                    ...
                                    <div className='preview-popup'>
                                        <div className='preview-popup-modal'>
                                            <div className='preview-popup-header'>
                                                ...
                                                <img src={closeIcon} style={{ cursor: "pointer" }} alt='close' onClick={e => this.previewPdfToggle("")} />
            ...
            /> */}
                            ...
                            <Card className="viewPatientForm">
                                <CardBody>
                                    <Row>
                                        <div className="form-group w-100 dis-contents">
                                            <Col xs="12">
                                                <h5 className="mt-2 w-100" >Peptide Status</h5>
                                                < hr className="w-100" />
                            ...
                            < hr /> */}
                                                            ...
                                                            <Row style={{ marginBottom: "3px" }}>
                                                                <Col xs="1">
                                                                    <Input
                                                                        ...
                                                                        onChange={() => {
                                                                            this.handlePeptidesCheckBoxChanges("pmrSentToClinic")
                                                                        }}
                                                                        checked={
                                                                            this.isPeptidesCheked("pmrSentToClinic")
                                                                        }
                                                                ...
                                                                <Col xs="3">
                                                                    <div className={`cus-date-picker `}>
                                                                        <DatePicker
                                                                            onChange={
                                                                                (date) => {
                                                                                    this.handlePeptidesDateChanges('pmrSentToClinic', date)
                                                                                }
                                                                            }
                                                                            selected={this.getPeptidesDate("pmrSentToClinic")}
                                                                            disabled={
                                                                ...
                                                                <Col xs="3">
                                                                    <button
                                                                        ...
                                                                        disabled={
                                                                            this.isPeptideDisabledForPMR() || this.getPeptidesDate("pmrSentToClinic") === ''
                                                                        }
                                                                        onClick={
                                                                            () => {
                                                                                this.updateDataAndRefreshPage(() => this.handlePmrSentToClinicSUbmit(this.state?.basicInfo?.peptideStatus?.pmrSendToCilnic && !this.state.basicInfo?.peptideStatus.clinicSendPMRToPharmacy))
                                                                            }
                                                                        }
                                                                    >
                                                                ...
                                                                <Col xs="1">
                                                                    <Input
                                                                        onChange={() => {
                                                                            this.handlePeptidesCheckBoxChanges("clinicSentPmrToPharmacy")
                                                                        }}
                                                                        checked={
                                                                            this.isPeptidesCheked("clinicSentPmrToPharmacy")
                                                                        }
                                                                ...
                                                                <Col xs="3 pt-2">
                                                                    <div className={`cus-date-picker `}>
                                                                        <DatePicker
                                                                            ...
                                                                            onChange={
                                                                                (date) => {
                                                                                    this.handlePeptidesDateChanges('clinicSentPmrToPharmacy', date)
                                                                                }
                                                                            }
                                                                            selected={this.getPeptidesDate("clinicSentPmrToPharmacy")}
                                                                            disabled={
                                                                ...
                                                                <Col xs="3 pt-2">
                                                                    <button
                                                                        ...
                                                                        disabled={
                                                                            this.isPeptideDisabledForClinic() || this.getPeptidesDate("clinicSentPmrToPharmacy") === ''
                                                                        }
                                                                        onClick={() => {
                                                                            this.updateDataAndRefreshPage(() => { return this.handleClinicSentPmrToPharmacy() })
                                                                        }}
                                                                    >
                                                                ...
                                                                <Col xs="1">
                                                                    ...
                                                                    <Input
                                                                        ...
                                                                        name="inline-checkbox1"
                                                                        onChange={() => {
                                                                            this.handlePeptidesCheckBoxChanges("peptidesReceivedByClinic")
                                                                        }}
                                                                        checked={
                                                                            this.isPeptidesCheked("peptidesReceivedByClinic")
                                                                        }
                                                                    />
                                                                </Col>
                                                                <Col xs="5" style={{ paddingTop: "0px" }}>
                                                                ...
                                                                <Col xs="3 pt-2">
                                                                    ...
                                                                    <div className={`cus-date-picker `}>
                                                                        <DatePicker
                                                                            ...
                                                                            onChange={
                                                                                (date) => {
                                                                                    this.handlePeptidesDateChanges('peptidesReceivedByClinic', date)
                                                                                }
                                                                            }
                                                                            selected={this.getPeptidesDate("peptidesReceivedByClinic")}
                                                                            disabled={
                                                                ...
                                                                <Col xs="3 pt-2">
                                                                    <button
                                                                        ...
                                                                        onClick={() => {
                                                                            this.updateDataAndRefreshPage(() => this.handlePeptidesReceviedByclinic())
                                                                        }}
                                                                    >
                                                                ...
                                                                <Col xs="1">
                                                                    <Input
                                                                        onChange={() => {
                                                                            this.handlePeptidesCheckBoxChanges("treatMentStarted")
                                                                        }}
                                                                        id="peptide_treatmntstrt"
                                                                        checked={
                                                                            this.isPeptidesCheked("treatMentStarted")
                                                                        }
                                                                ...
                                                                <Col xs="3 pt-2">
                                                                    ...
                                                                    <div className={`cus-date-picker `}>
                                                                        <DatePicker
                                                                            ...
                                                                            onChange={
                                                                                (date) => {
                                                                                    this.handlePeptidesDateChanges('treatMentStarted', date)
                                                                                }
                                                                            }
                                                                            selected={this.getPeptidesDate("treatMentStarted")}
                                                                            disabled={
                                                                ...
                                                                <Col xs="3 pt-2">
                                                                    <button
                                                                        ...
                                                                        onClick={() => {
                                                                            this.updateDataAndRefreshPage(() => this.handlePeptidesTreatmentStarted())
                                                                        }}
                                                                    >
                                                                        <i className="fa fa-dot-circle-o text-light"></i> Submit
                                                                    </button>
                                                ...
                                                </Col> : <FormGroup className="ml-3">This section will be visible after completion of PMR & Final Report.</FormGroup>}
                                        </div>
                              ...
                              Payments
                                        ...
                                        <TabContent activeTab={this.state.activeTab[0]}>
                                            <TabPane tabId="2">
                                                <button
                                                    className="btn btn-primary btn-md"
                                                    style={{ marginBottom: "5px", float: "right", display: this.state.isEdit ? "block" : "none" }}
                                                    onClick={() => this.handleShowDiagnostic()}
                                                >
                                                    Add{" "}
                                                </button>
                                                <Table responsive bordered key="tblPDisease" className="table-hover">
                                                                                    ...
                                                                                    <Link
                                                                                        ...
                                                                                        to="#" onClick={() => {
                                                                                            this.setState({
                                                                                                ...
                                                                                                diagnosticAnalysis: data.diagnosticAnalysis,
                                                                                                outcome: data.outcome,
                                                                                                patientDiagnosticId: data.patientDiagnosticId
                                                                                            });
                                                                                        }}>
                                                                                        <i className="icon-pencil"></i>
                                                                                    </Link>
                                                                                    ...
                                                                                    >
                                                                                        {(confirm) => (
                                                                                            <Link
                                                                                                className="card-header-action btn btn-close"
                                                                                                to="#"
                                                                                                onClick={confirm((e) => this.DeletePatientDiagnostic(e, data.patientDiagnosticId))}>
                                                                                                <i className="icon-trash"></i>
                                                                                            </Link>
                                                                                        )}
                                                                                    </Confirm>
                                            ...
                                            <TabPane tabId="3">
                                                ...
                                                <Table responsive bordered key="tblPPrescription" className="table-hover">
                                                    <thead className="thead-light">
                                                        <tr>
                                                            <th>S.No.</th>
                                              ...
                                              />
                                                                                    ...
                                                                                    >
                                                                                        {(confirm) => (
                                                                                            <Link
                                                                                                className="card-header-action btn btn-close"
                                                                                                to="#" onClick={confirm((e) => this.DeletePatientprescription(e, data.patientPrescriptionId))}>
                                                                                                <i className="icon-trash"></i>
                                                                                            </Link>)}
                                                                                    </Confirm>
                                            ...
                                            <TabPane tabId="1" id="BottomSectionTreatments">
                                                ...
                                                <Table responsive bordered key="tblPTreatmentReport" className="table-hover">
                                                    <thead className="thead-light">
                                                        <tr>
                                                            <th>S.No.</th>
                                                                                    ...
                                                                                    >
                                                                                        {(confirm) => (
                                                                                            <Link
                                                                                                ...
                                                                                                className="card-header-action btn btn-close"
                                                                                                to="#" onClick={confirm((e) => this.DeletePatienttreatmentreport(e, data.patientTreatmentReportId))}>
                                                                                                <i className="icon-trash"></i>
                                                                                            </Link>)}</Confirm>
                                            ...
                                            <TabPane tabId="4">
                                                <Table responsive bordered key="tblPEmergenCnts" className="table-hover">
                                                    <thead className="thead-light">
                                                        <tr>
                                                            <th>Name</th>
                                                            <th>Email</th>
                                            ...
                                            <TabPane tabId="5">
                                                <Table responsive bordered key="tblPInsurance" className="table-hover">
                                                    <thead className="thead-light">
                                                        <tr>
                ...
                <Modal isOpen={this.state.showPmrVerify} className="modal-dialog modal-xl">
                    <ModalHeader toggle={() => {
                        errors.notes = ''
                        errors.reviseReason = ''
                        errors.version = ''
                                            ...
                                            <div className="col-12">
                                                ...
                                                <Input type="text"
                                                    onChange={(e) => {
                                                        ...
                                                        if (isNumber(e.target.value) || e.target.value.trim() === '') {
                                                            e.target.value = e.target.value.trim()
                                                            this.handleInputChange(e, 'version')
                                                        }
                                                    }}
                                                    readOnly={!this.state.isRevised} placeholder="Version" name="Version" value={this.state?.version || ''} />
                                        ...
                                        {this.state.isRevised ?
                                            <>
                                                <div className="form-group my-4 col-md-12">
                                                    ...
                                                    <div className="col-12">
                                                        <Input className="is-valid" disabled={!this?.state?.isRevised} type="text" tabIndex="2" placeholder="Enter Revised Reason" onChange={(e) => { this.handleInputChange(e, 'reviseReason') }} />
                                                        {<span className='error'>{errors.reviseReason}</span>}
                                                    </div>
                                                </div>
                                            </> : null}
                                    </div>
                                    {!this.state.isRevised ? <div className="text-right d-flex justify-content-end">
                                        <Button type="button" color="primary"
                                            ...
                                            style={{ "marginRight": "5px", display: this.state.isEdit ? "block" : "none" }}><i className="fa fa-dot-circle-o"></i> Verify</Button>
                                        <Button type="button" color="primary"
                                            onClick={() => {
                                                this.setState({
                                                    isRevised: true,
                                            ...
                                            style={{ display: this.state.isEdit ? "block" : "none" }}
                                        ><i className="fa fa-dot-circle-o"></i> Revision</Button>
                                    </div> : <div className="form-group my-4 col-md-12 text-right d-flex justify-content-end">
                                        <Button type="button" color="primary" onClick={() => {

                                            this.revisedPmr()
                                        }}
                                            style={{ "marginRight": "5px", display: this.state.isEdit ? "block" : "none" }}>
                                        ...
                                        <Button type="button" color="secondary" onClick={() => {
                                            ...
                                            this.setState({
                                                isRevised: false,
                                                version: this?.state?.patientPMRReports?.[this?.state?.PMRINDEX]?.pmrCurrentVersion,
                                                notes: this?.state?.patientPMRReports?.[this?.state?.PMRINDEX]?.notes || ``,
                                                errors
                                            })
                                        }} style={{ "marginRight": "5px", display: this.state.isEdit ? "block" : "none" }}><i className="fa fa-dot-circle-o"></i> cancel </Button>
                                    </div>}
                                </div>
                            </div>
                        </Form>
                ...
                <Modal isOpen={this.state.showPmrApprove} className="modal-dialog modal-xl">
                                ...
                                <div className="col-md-12">
                                    <div className="form-group row my-4">
                                        <div className="col-md-6">
                                            <label htmlFor="date" className="col-12 col-form-label">Version</label>
                                            <div className="col-12">
                                                <Input type="text" readOnly="true" placeholder="Version" name="Version" value={this.state.version} />
                                            </div>
                                    ...
                                    <div className="text-right">
                                        <Button type="button" color="success"
                                            ...
                                            style={{ "marginRight": "5px" }}><i className="fa fa-dot-circle-o"></i> Approve</Button>
                                        ...
                                        ><i className="fa fa-dot-circle-o"></i> Reject</Button>
                                    </div>
                                </div>
                            </div>
                        </Form>
                ...
                <Modal isOpen={this.state.showPaidStatus} className="modal-dialog modal-md">
                                        ...
                                        <div className="col-md-12">
                                            ...
                                            <div class="form-check-inline w-100">
                                                <label class="form-check-label">
                                                    <Input type="radio" class="form-check-input" onChange={(e) => { this.handleInputChange(e, 'paymentType') }} name="paymentType" value={'y'} />Full Payment
                                                </label>
                                            </div>
                                            <div class="form-check-inline w-100">
                                                <label class="form-check-label">
                                                    <Input type="radio" class="form-check-input" checked={this.state.paymentType == "n"} name="paymentType" onChange={(e) => { this.handleInputChange(e, 'paymentType') }} value={'n'} />Partial Payment
                                                </label>
                                            </div>
                          ...
                          <Input type="radio" name="Version" value={'n'} />
                                    ...
                                    <div className="text-right">
                                        <Button color="primary"
                                            ...
                                            style={{ "marginRight": "5px" }}><i className="fa fa-dot-circle-o"></i> Submit</Button>
                                        <Button className="btn  btn-secondary"
                                            onClick={() => {
                                                this.setState({
                ...
                <Modal isOpen={this.state.showConfirmPayment} className="modal-dialog modal-md">
                                ...
                                <div className="col-md-12">
                                    <div className="form-group row my-4">
                                        <div className="col-md-12">
                                            ...
                                            Please confirm, did you received {this.state.paymentType === 'y' ? `Full` : "Partial"} payment ?
                                    ...
                                    <div className="text-right">
                                        <Button type="button"
                                            ...
                                            style={{ "marginRight": "5px" }}><i className="fa fa-dot-circle-o"></i> Yes</Button>
                                        <Button type="button"
                ...
                <Modal isOpen={showdisease} className="modal-dialog modal-md">
                    ...
                    <ModalBody>
                        ...
                        >
                            <div className="form-group">
                                ...
                                <Autocomplete
                                    ...
                                    filterOptions={(options, params) => {
                                        ...
                                        let filtered = this.filterDisease(options, params.inputValue)
                                    ...
                                    options={dieseasData.filter(e => e.diseaseCatId == this.state?.diseaseCategoryId)}
                                    ...
                                    sx={{ width: 300, border: (this.state?.updateDisease || this.state?.diseaseId == "0") ? "1px solid #4dbd74" : "none", borderRadius: "10px" }}
                                    freeSolo
                                    renderInput={(params) => (
                                ...
                                {this.state?.updateDisease && <span style={{ color: "#4dbd74" }}>{this.state.updateDiseaseTo} will be updated to {this.state?.currentDiseaseName}</span>}
                                {<span className="success" style={{ color: "#4dbd74" }}>{this.state?.diseaseId == "0" && `A new Disease "${this.state.currentDiseaseName}" will be created !`}</span>}
                            </div>
                            ...
                            {(this.state.callPipeline == true || this.state.recallPipeline == true) ? <div className="form-group" style={{ display: [2, 3, 5].includes(Number(this.state.diseaseCategoryId)) ? "block" : "none" }}>



                                <label htmlFor="recipient-name" className="form-control-label">
                                    use RNA Blood instead of RNA Tumor and ctDNA instead of WES Tumor (for inoperable patients)
                                </label>
                                <hr />
                                ...
                                <div class="custom-control custom-radio mb-2">
                                    <input id="yespipeline" type="radio" className="custom-control-input" disabled checked={this.state?.diseaseCategoryId == 2} />
                                    <label for="yespipeline" className="custom-control-label" style={{ "fontWeight": 500 }}>Yes</label>
                                </div>

                                <div class="custom-control custom-radio mb-2">
                                    <input id="Nopipeline" type="radio" className="custom-control-input " disabled checked={this.state?.diseaseCategoryId == 3} />
                                    <label for="Nopipeline" className="custom-control-label" style={{ "fontWeight": 500 }}>No</label>
                                </div>
                    ...
                    <ModalFooter>
                        ...
                        >
                            ...
                            {(this.state.recallPipeline == false && this.state.callPipeline == false) && "Update"}
                        </Button>
                    </ModalFooter>
                </Modal>

                <Modal isOpen={this.state.showdiseaseDescription} className="modal-dialog modal-lg">
                    ...
                    <ModalFooter>
                        <Button color="secondary" onClick={this.handleCloseDiseaseDescription}>
                            Close
                        </Button>
                        <Button color="primary"
                            disabled={this.state.DiseaseDescData.trim() == ""}
                ...
                <Modal isOpen={this.state.showUpdateStatusModel} className="modal-dialog modal-md">
                    <ModalHeader>Change Status </ModalHeader>
                    <ModalBody>
                        <div className="form-group">

                            Are you sure you want to change the status to  <b>{this.state.newStatus} </b>?
                        </div>
                ...
                <Modal isOpen={UploadFile} className="modal-dialog modal-md">
                    <ModalHeader> {_.capitalize(String(this.state?.UploadFileFor))}</ModalHeader>
                    <ModalBody>
                        <div className="form-group">
                            <label htmlFor="recipient-name" className="form-control-label">Upload File</label>
                            <label htmlFor="FileUpload" className="fileuploadlabel" onDragEnter={this.handleDrag} onDrop={this.handleDrop} onDragLeave={this.handleDrag} onDragOver={this.handleDrag}>
                                {this.state?.file?.name ? this.state?.file?.name : this.state?.drag ? "Drop file here to Upload" : <p>Drag and drop your file here or Click to Upload a file </p>}
                                <Input type="file" id="FileUpload" name="UploadFile" className="form-control fileuploadinput" tabIndex="1" multiple={false} onChange={this.handleFileChange} />
                            </label>

                            {/* <a href={demoImportFile} download>Download Import Demo File</a> */}
                            {uploadError?.length > 0 ? (
                                <span className="error">{uploadError}</span>
                            ) : this.state.file?.name ? <div style={{
                                "cursor": "pointer",
                                "color": "blue",
                ...
                <Modal isOpen={showngslaboratory} className="modal-dialog modal-md left-modal">
                ...
                <div className="modl-left modal-left">
                    <Modal isOpen={showdiagnostic} className="modal-dialog modal-lg left-modal" >
                        ...
                        <ModalBody className="" style={{ backgroundColor: 'white' }}>
                            ...
                            <div className="form-group">
                                ...
                                <Input
                                    ...
                                    tabIndex="2"
                                    rows="5"
                                    className={errors.diagnosticAnalysis ? "is-invalid" : "is-valid"}
                ...
                <div className="modl-left modal-left">
                    <Modal isOpen={this.state.showeditdiagnostic} className="modal-dialog modal-lg left-modal" >
                                                                    ...
                                                                    return (data2.patientDiagnosticId
                                                                        ...
                                                                        <tr key={i2}>
                                                                            ...
                                                                            <td >
                                                                                ...
                                                                                <Link to="#" onClick={e => this.toggleDiagonosticFile(e, data2.patientDiagnosticFileId)} >
                ...
                <Modal isOpen={showprescription} className="modal-dialog modal-lg left-modal">
                    ...
                    <ModalBody className="modal-body_left_modal">
                        ...
                        <div className="form-group">
                            <label htmlFor="recipient-name" className="form-control-label">
                                Prescription Description <span className="requiredField">*</span>
                            </label>
                            <Input
                                type="textarea"
                                ...
                                tabIndex="2"
                                rows="5"
                                className={errors.prescriptionDescription ? "is-invalid" : "is-valid"}
                ...
                <Modal isOpen={this.state.showeditprescription} className="modal-dialog modal-lg left-modal">
                    <ModalHeader>Edit Prescription</ModalHeader>
                    <ModalBody className="modal-body_left_modal" style={{ backgroundColor: 'white' }}>
                        <div className="form-group">
                            <label htmlFor="recipient-name" className="form-control-label">
                                Prescription Date
                            </label>
                                                            ...
                                                            return (data2.patientPrescriptionId
                                                                ...
                                                                <tr key={i2}>
                                                                    ...
                                                                    <td >
                                                                        ...
                                                                        <Link to="#" onClick={(e) => this.togglePrescriptionFile(e, data2.patientPrescriptionFileId)}>
                                                                            <i className="icon-trash" />
                                                                        </Link>
                                                                    </td>
                                                                </tr> : "")
                ...
                <Modal isOpen={showtreatment} className="modal-dialog modal-lg left-modal">
                    ...
                    <ModalBody style={{ backgroundColor: 'white' }}>
                        ...
                        <div className="form-group">
                            ...
                            <Input
                                ...
                                rows="5"
                                maxLength='2000'
                                className={errors.treatmentDescription ? "is-invalid" : "is-valid"}
                ...
                <Modal isOpen={this.state.showedittreatment} className="modal-dialog modal-lg left-modal">
                                        ...
                                        <>
                                            {data?.patientTreatmentReportFile?.length > 0 ? (
                                                data?.patientTreatmentReportFile?.map((data2, i2) => {
                                                    return data2.patientTreatmentReportId == this.state.patientTreatmentReportId &&
                                                        ...
                                                        <tr key={i2}>
                                                            ...
                                                            <td className=''>
                                                                ...
                                                                <Link
                                                                    to='#'
                                                                    onClick={(e) =>
                                                                        this.toggleTreatementsFile(e, data2.patientTreatmentreportFileId)
                                                                    }>
                                                                    <i className='icon-trash' />
                                                                </Link>
                                                            </td>
                ...
                </Modal>
                {
                    ...
                    <div style={{ marginLeft: "36%" }}>
                        <Modal isOpen={this.state.DeletemodalfortreatmentsFile} style={{ width: "500px" }} >
                            <ModalHeader>Confirm</ModalHeader>
                            <ModalBody>Are you sure you want to delete this file?</ModalBody>
                            <ModalFooter>
                                <Button outline color="danger" onClick={e => this.DeleteTreatement_file(e, this.state.deletetreatmentsFileId)}>Delete</Button>
                                <Button color="secondary" onClick={this.toggleTreatementsFile}>Cancel</Button>
                            </ModalFooter>
                        </Modal>
                    </div>
                }
                ...
                <Modal isOpen={this.state.analysisModal} className="modal-dialog modal-md h-500"  >
                    <ModalHeader toggle={() => {
                        this.setState({
                            analysisModal: false,
                            analysisText: '',
