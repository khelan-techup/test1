...
class List extends Component {
  ...
  getListData(pageNo) {
    ...
    const url = apiroute + '/api/BE_Institute/GetAllPaging';
      ...
      .then(result => {
        if (result.data.flag) {
          ...
          this.setState({
            ...
            institutes: result.data.outdata, loading: false
          })
        } else {
  ...
  deleteRow(e, id) {
    ...
    const apiroute = window.$APIPath;
    const url = apiroute + '/api/BE_Institute/Delete?id=' + id + '&userId=' + userId + '';
  ...
  render() {
    ...
    const { loading, institutes, currentPage, currentIndex, pagesCount, pageSize, authError, error } = this.state;
    return (
      <div className="animated fadeIn">
        {this.loader()}
        <Row className="mb-3">
          <Col xs="10" lg="10">
            <h5 className="mt-2"><i className="fa fa-align-justify"></i> Institution List</h5>
          </Col>
          <Col xs="2" lg="2">
            {this.state.isEdit ?
              <Link to="/institutes/details">
                <button className="btn btn-primary btn-block">Add Institution</button>
              </Link>
              : null}
          </Col>
        </Row>
                  ...
                  <tbody>
                    {
                      ...
                      institutes.length > 0 ? (
                        institutes
                          ...
                          .map((data, i) => {
                            return (<tr key={i}>
                              ...
                              {this.state.isEdit && this.state.isView ?
                                <td>
                                  <Link className="btn btn-success btn-sm btn-pill" to={'/institutes/info/' + data.instituteId}><b>View</b></Link>{" "}
                                  {this.state.isEdit ?
                                    <React.Fragment>
                                      <Link className="btn btn-primary btn-sm btn-pill" to={'/institutes/modify/' + data.instituteId}><b>Edit</b></Link>{" "}
                                      {/*<Link className="btn btn-danger btn-sm btn-pill" to="#" onClick={e => this.deleteRow(e, data.instituteId)}><b>Delete</b></Link>*/}
                                    </React.Fragment>
                                    : null}
                                </td>
                                : null}
                              {this.state.isView ?
                                <td>
                                  <Link className="btn btn-dark btn-sm btn-pill" to={'/institutes/patients/' + data.instituteId}>Patients</Link>
                                  {" "}
                                  <Link className="btn btn-dark btn-sm btn-pill" to={'/institutes/practitioners/' + data.instituteId}>Practitioners</Link>
                                </td>
                                : null}
                              <td>
                                {data.isCognito ?
                        ...
                        <tr>
                          <td colSpan="7" className="tdCenter">No institutes.</td></tr>
                        // )) : (
                        // <tr>
