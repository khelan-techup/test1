...
class PatientReportDetail extends Component {
  ...
  onSelect = (selectedList, selectedItem, index) => {
    ...
    console.log(selectedList, selectedItem, index);
    this.state.geneHallmarkData.push(selectedItem.id);
    this.setState({
      geneHallmarkData:
        this.state.geneHallmarkData.push(selectedItem.hallMarkNo),
      // hallMarkErrs: errClone
    })
  ...
  onRemove = (selectedList, removedItem, index) => {
    console.log(selectedList, removedItem, index);
  ...
  getData = (id, aid) => {
    ...
    if (param.id != undefined && param.aid != undefined && param.did != undefined) {
      ...
      // /api/BE_PatientReport/GetPatientReportData
      const url = apiroute + '/api/BE_PatientReport/GetPatientReportData?id=' + id + '&aid=' + aid;
      let setvalues = this?.state?.geneHallmarkData?.map((data) => {
        let newdata = data?.hallMarkNo?.split(",")
        return newdata?.map((data) => {
          return { hallMarkNo: data }
        })
      })
      ...
      axiosInstance.get(url, {
        ...
        .then(result => {
          if (result.data.flag) {
            ...
            let hallmarkData = rData?.hallmarkRefNos.map((data) => {
              return { hallMarkNo: data }
            })
            ...
            this.setState({
              ...
              // LastSummaryNote: rData?.lastSummaryNote === null ? "<p></p>" : rData?.lastSummaryNote,
              PRGeneticCharacteristics: rData?.prGeneticCharacteristics,
              Multiselectvalues: [...hallmarkData],
              geneSelectedvalues: [...setvalues],
              // PRImmuneCheckpoint: rData?.prImmuneCheckpoint,
              // PRDrugsIdentified: rData?.prDrugsIdentified,
              // PRNaturalAgents: rData?.prNaturalAgents,
              // PRRegenerativePeptides: rData?.prRegenerativePeptides,
  ...
  addGeneric(e) {
    ...
    if (gTitle != undefined && gTitle != "" && gDesc != undefined && gDesc != "") {
  ...
  validateForm = (errors) => {
    ...
    if (this.state.PRGeneticCharacteristics.length <= 0) {
      ...
      if (
        ...
        lastData.title === "" || lastData.title === null || lastData.description === "" || lastData.description === null
      ) {
      ...
      <div className="animated fadeIn">
        ...
        <Row className="mb-3">
          ...
          <Col xs="2" lg="2">
            ...
            <Link to={this?.props?.location?.state?.redirectTo ? "#" : '/patients/list'}>
              ...
              <CardBody>
                <Form onSubmit={this.handleSubmit.bind(this)}>
                  ...
                  <div className="row">
                                        ...
                                        <div className="form-group row my-4">
                                          ...
                                          <div className="col-md-10">
                                            <CKEditor
                                              ...
                                              config={{ placeholder: "Enter description for genetic characteristics here" }}
                                              //onReady={editor => {
                    ...
                    <div className="col-md-12">
                      <div className="form-group my-4 mx-0 col-md-12 text-center">
                        <Button onClick={() => { this.handleShowTableGene() }} color="primary"> Load Gene Hallmarks</Button>{" "}
                        <Button style={{ float: "right" }} disabled={loading || this.state.isSubmitEnabled} type="submit" color="primary"><i className="fa fa-dot-circle-o"></i> Submit</Button>{" "}
                      </div>
              ...
              <tbody>
                ...
                {geneHallmarkData.length > 0 ?
                  geneHallmarkData?.map((data, index) => {
                    ...
                    // let datadrp = hallmarkRefNos?.map((datahall) => {
                    //   return { hallMarkNo: datahall }
                    // })
                    // this.setState({
                    //   Multiselectvalues: datadrp
                    // })

                    return <tr key={index}>
                      <td style={{ width: "20%" }}>{data?.geneName}</td>
                      <td style={{ width: "20%" }}>

                        <Multiselect
                          options={this.state.Multiselectvalues} // Options to display in the dropdown
                          selectedValues={this.state.geneSelectedvalues[index]} // Preselected value to persist in dropdown
                          onSelect={(selectedList, selectedItem) => {
                            this.onSelect(selectedList, selectedItem, index)
                          }}  // Function will trigger on select event
                          onRemove={(selectedList, selectedItem) => {
                            this.onSelect(selectedList, selectedItem, index)
                          }}// Function will trigger on remove event
                          displayValue="hallMarkNo" // Property name to display in the dropdown options
                          placeholder="Select hallMark No"
                          tabIndex="4"
                          name="hallMarkNo"
                        />
                      ...
                      <td style={{ margin: "0" }}><textarea
                        ...
                        rows={3} value={data?.hallMarkReference} />
        ...
        <Modal isOpen={showGenetic} className="modal-dialog modal-md right-modal">
          ...
          <ModalBody className="modal-body_right_modal">
            <div className="form-group">
              <label htmlFor="recipient-name" className="form-control-label">Gene Name</label>
            ...
            <div className="form-group">
              ...
              <CKEditor
                editor={ClassicEditor}
                // data={gdata.description || ""}
                config={{ placeholder: "Enter a description" }}
                //onReady={editor => {
                // You can store the "editor" and use when it is needed.
                //console.log('Editor is ready to use!', editor);
                //}}
