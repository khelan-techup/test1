...
import { BE_OrganizationUser_UpdateTooltipSteps, BE_Role_Delete, BE_Role_GetAllPaging } from '../../../common/allApiEndPoints';
class List extends Component {
  constructor(props) {
    ...
    this.initialState = {
      ...
      steps: [{
        element: "#pagetitle",
        title: 'Role List',
        intro: "Used to add or update Role ",
      ...
      },
      {
        ...
        title: 'Add New Role',
        tooltipClass: "cssClassName1",
        intro: "You can add new role by clicking on this Add button."
      },
      {
        element: "#activeInactiveFilter",
        title: 'Active/Inactive filter for role list',
        intro: "You can filter role list by selecting active or inactive option.",
        tooltipClass: "cssClassName1",
      },
      {
        element: "#searchbar ",
        title: 'Search Role',
        tooltipClass: "cssClassName1",
        intro: "This Search Bar allows the User to search in the role list."
      },

      {
        ...
        title: 'Edit Role',
        tooltipClass: "cssClassName1",
        intro: "You can edit or update role details by clicking on this Edit button."
      },
      {
        element: "#help",
        tooltipClass: "cssClassName1",
  ...
  getListData(pageNo) {
    ...
    const apiroute = window.$APIPath;
    // const url = apiroute + '/api/BE_Role/GetAllPaging';
    const url = apiroute + BE_Role_GetAllPaging;
    ...
    axiosInstance.post(url, data, {
      ...
      .then(result => {
        ...
        if (result.data.flag) {
          ...
          //var finaldata = rdata.filter(role => role.roleName !== "SuperAdmin");
          var finaldata = rdata;
          this.setState({
  ...
  handleChange = (e) => {
    ...
    // if (value == true) {
    //   let countT = this.state.roles.filter((data, i) => {
    ...
    // }
    // if (value == false) {
    //   let countF = this.state.roles.filter((data, i) => {
  ...
  deleteRow = (e, id) => {
    ...
    this.setState({ loading: true });
    const apiroute = window.$APIPath;
    // const url = apiroute + '/api/BE_Role/Delete?id=' + id + '&userId=' + userId + '';
    const url = apiroute + BE_Role_Delete(id, userId)
    ...
    axiosInstance.delete(url, {
      ...
      .then(result => {
        if (result.data.flag) {
          ...
          //this.setState({
          //  roles: currroles.filter(role => role.roleId !== id)
          //});
          this.getListData(0);
        } else {
          this.setState({ loading: false });
  ...
  render() {
    ...
    const { loading, roles, currentPage, currentIndex, pagesCount, pageSize, authError, error } = this.state;
    return (
      <div className="animated fadeIn">
        ...
        <Row className="mb-3">
          <Col xs="11" lg="11">
            <h5 className="mt-2" id="pagetitle" style={{ width: "fit-content" }}><i className="fa fa-align-justify"></i> Role List</h5>
          </Col>
          <Col xs="1" lg="1">
            {this.state.isEdit ?
              <Link to="/master/role/details">
          ...
          <Col xs="12" lg="12">
            <Card>
              <CardHeader>
                <Row>
                  <Col xs="2" >
                    <Input type="select" id="activeInactiveFilter" name="slDelete" onChange={this.handleChange}>
                      <option value="true">
                        {/* Active {this.state.roles.length>0?`(${this.state.activeCountTrue})`:''} */}
                        Active {this.state.tempCount == true ? `(${this.state.activeCountTrue})` : ''}
                      </option>
                  ...
                  <tbody>
                    {
                      ...
                      roles.length > 0 ? (
                        roles
                          ...
                          .map((data, i) => {
                            return (<tr key={i}>
                              ...
                              <td className="thNone">{data.roleId}</td>
                              {/* {this.state.isEdit ? */}
                              <td>
                                <Link id="Edit" className="btn btn-primary btn-sm btn-pill" to={'/master/role/modify/' + data.roleId}>Edit</Link>{" "}
                                {/*<Link className="btn btn-danger btn-sm btn-pill" to="#" onClick={e => this.deleteRow(e, data.roleId)}>Delete</Link>*/}
                                {/*<Confirm title="Confirm" description="Are you sure?">*/}
                                {/*  {confirm => (*/}
                                {/*    <Link className="btn btn-danger btn-sm btn-pill" to="#"  onClick={confirm(e => this.deleteRow(e, data.roleId))}>*/}
                                {/*      Delete</Link>*/}
                                {/*  )}*/}
                                {/*</Confirm>*/}
                              </td>
                        ...
                        <tr>
                          <td colSpan="3" className="tdCenter">No roles.</td></tr>
                        // )) : (
                        // <tr>
